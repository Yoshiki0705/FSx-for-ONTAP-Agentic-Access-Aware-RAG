...
export class RegionConfigManager {
  ...
  static getRegionSelectOptions(): RegionSelectOption[] {
    // モデル数計算用のインポート
    const { PROVIDER_REGION_AVAILABILITY } = require('./region-model-availability');
    ...
    return REGION_CONFIGS.map(config => {
      ...
      // modelCountが0の場合は最低1に設定（エラー防止）
      if (modelCount === 0 && config.bedrockSupported) {
        modelCount = 1;
      }
      ...
      return {
        ...
        modelCount: modelCount || 0, // undefinedを防ぐ
