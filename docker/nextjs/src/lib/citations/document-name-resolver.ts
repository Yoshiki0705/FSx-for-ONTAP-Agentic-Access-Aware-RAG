...
export function resolveDocumentDisplay(s3UriOrKey: string, locale: 'ja' | 'en' = 'ja'): {
  ...
  const key = extractKey(s3UriOrKey);
  const filename = key.split('/').pop() || key;
  const isUuidBased = UUID_PATTERN.test(filename);
...
/**
 * Extract S3 key from URI or return as-is.
 */
function extractKey(s3UriOrKey: string): string {
  let key = s3UriOrKey;
  if (key.includes('s3://')) {
    const parts = key.replace('s3://', '').split('/');
...
/**
 * Build a complete citation display info for the UI.
 * Provides file path, human-readable name, and FSx volume path.
 */
export function buildCitationDisplayInfo(s3UriOrKey: string, metadata?: Record<string, unknown>, locale: 'ja' | 'en' = 'ja'): {
  /** Human-readable display name */
  displayName: string;
  /** Relative file path on FSx volume (for tooltip / link) */
  filePath: string;
  /** Full FSx volume path (e.g., /vol1/industry-packs/healthcare/...) */
  fsxVolumePath: string;
  /** Permission badge text */
  permissionBadge: string | null;
  /** Whether the file is UUID-based (auto-generated) */
  isAutoGenerated: boolean;
} {
  const key = extractKey(s3UriOrKey);
  const filename = key.split('/').pop() || key;
  const isAutoGenerated = UUID_PATTERN.test(filename);
  const displayName = resolveDocumentName(s3UriOrKey, locale);

  // Permission badge from metadata
  let permissionBadge: string | null = null;
  if (metadata) {
    const sids = metadata.allowed_group_sids;
    if (sids) {
      const sidStr = typeof sids === 'string' ? sids : Array.isArray(sids) ? sids.join(',') : '';
      if (sidStr.includes('S-1-1-0')) {
        permissionBadge = locale === 'ja' ? '全員アクセス可' : 'Everyone';
      } else if (sidStr.includes('-512')) {
        permissionBadge = locale === 'ja' ? '管理者のみ' : 'Admin Only';
      } else {
        permissionBadge = locale === 'ja' ? '制限付き' : 'Restricted';
      }
    }
  }

  return {
    displayName: isAutoGenerated ? displayName : filename,
    filePath: key,
    fsxVolumePath: `/vol1/${key}`,
    permissionBadge,
    isAutoGenerated,
  };
}
