...
| `automation/*/tests/`, `lambda/kb-auto-sync/tests/` | pytest + hypothesis + moto | `make test-python` | 118 tests across three directories. Each has its own `.venv`; `make test-python` creates it when missing, because the target previously called the system `python3` and failed on a collection error wherever `hypothesis` was absent |
