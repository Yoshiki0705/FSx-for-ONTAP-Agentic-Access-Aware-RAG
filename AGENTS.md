...
# 上記 10 通りが CI の synth-matrix ジョブ（.github/workflows/ci-cd.yml）のレーンと同一。
...
# Frontend tests — the two runners own different directories:
#   Jest   : route handlers and utils (everything except src/__tests__)
#   Vitest : src/__tests__ (property and unit tests)
...
# ドキュメントの相対リンク切れ検査（CI: doc-links ジョブ）
...
- **AgentCore Web Search (Option A — us-east-1 stack)**: Web Search Tool runs in **us-east-1 only**. When `enableWebSearch=true` (requires `enableAgentCoreGateway=true`), a dedicated `DemoWebSearchGatewayStack` is synthesized in us-east-1 using `crossRegionReferences`, creating the Gateway + Web Search target via `AwsCustomResource` with the **[verified 2026-06-18 / us-east-1]** connector shape (`mcp.connector.source.connectorId: "web-search"`, PoC §9.1). The Gateway URL is exported cross-region and injected into the ap-northeast-1 WebApp Lambda as `WEB_SEARCH_GATEWAY_URL` (mechanism C). The ap-northeast-1 `AgentCoreGateway` construct still emits a synth warning and does **not** host the target (region mismatch). Mechanism A (Claude Platform `callWithWebSearch` via `ENABLE_WEB_SEARCH`) remains region-agnostic and independent. The Option B PoC/teardown scripts lived in a local workspace and are **not shipped in this repository**; reproduce with `npx cdk deploy '*-WebSearchGateway' -c enableWebSearch=true -c enableAgentCoreGateway=true`.
...
- **AgentCore Optimization (Preview)**: **[hypothesis]** `createConfigurationBundle` params via `AwsCustomResource` are **unverified** — inferred from the SDK shape, not confirmed against the service — opt-in only (`enableAgentOptimization=false` default). CDK provisions Config Bundle + IAM role; Recommendations/A/B tests run via agentcore CLI/SDK
...
- **Bedrock KB Chunking**: Changing `kbChunkingStrategy` requires DataSource re-sync (re-ingestion). An invalid value **throws at synth time** — it is not silently coerced to `FIXED_SIZE`, because the CfnOutput would then show the typo while the emitted configuration said otherwise. Case is normalized (`semantic` → `SEMANTIC`), and both `KbChunkingStrategy` and `KbChunkingConfig` are derived from the same converted value
...
# Build+push the Next.js image to ECR, then update the Lambda function.
# Full command sequence: docs/operations-runbook.md §3 "WebApp Docker Build & Deploy"
...
| Scheduled `Continuous Monitoring` failed intermittently on main | `fc.date()` generates `new Date(NaN)` unless `noInvalidDate: true`, and `evaluateSchedule` threw `RangeError` from `toISOString()` — a seed-dependent failure that looked like flakiness but was a real crash on invalid input | Guard invalid `Date` before formatting and return a denial (`reason: 'invalid_time'`). When a property test fails only sometimes, read the counterexample before calling it flaky |
...
| A workflow reported success while doing nothing | It called a script under `development/` (gitignored, so absent in CI) and either printed "not found, skipping" or fell through to a hidden fallback. `document-auto-update.yml` did this twice per push; three pipelines ran the same 763 tests through such a fallback | `scripts/check-workflow-paths.py` fails when a workflow references a repo path that does not exist. Intentional absence is marked `allow:missing-path` on that line |
| Python tests passed locally but ran nowhere in CI | The three pytest directories were only reachable through `make test-python`, and that target could not run on a machine without the dependencies installed | A `python-tests` CI job runs all three on Python 3.12, wired into `quality-gate`. Same class of gap as the frontend runners |
| Frontend Jest reported 90 failed suites | `jest.config.js` `testMatch` also matched `src/__tests__/`, which Vitest owns, so Jest tried to run 121 files that import from `vitest`. Neither runner was invoked in CI, so nobody saw it | `testPathIgnorePatterns` now excludes `src/__tests__/` and `src/__tests__.disabled/`, and a `frontend-tests` CI job runs both runners |
| `ReferenceError: Request is not defined` in a route handler test | jsdom has no Web `Request` global | Add `@jest-environment node` in the file's docblock, as the other route tests do |
...
| `docker/nextjs` outside `src/__tests__/` | Jest | `npx jest --no-coverage` | Route handlers and utils. `jest.config.js` ignores `src/__tests__/` so the two runners do not collide |
...
| `automation/*/tests/`, `lambda/kb-auto-sync/tests/`, `tests/permission-matrix/` | pytest + hypothesis + moto | `make test-python` | 149 tests across four directories. Each has its own `.venv`; `make test-python` creates it when missing, because the target previously called the system `python3` and failed on a collection error wherever `hypothesis` was absent |
...
- **記録より細かい精度を書かない。** 日を記録していないなら `YYYY-MM` のまま書く。
...
- **翻訳が原文の一部しか訳していない場合は開示バナーを付ける**（`<!-- i18n:abridged -->` + 読者向けの 1 行）。ティアと検査は [多言語ティア方針](docs/i18n-policy.md) / `scripts/check-i18n.py`。未翻訳のファイルはティア 3 の範囲で、プレースホルダのページを作らない。
- **ただし「翻訳ディレクトリに置かれた原文のコピー」は未翻訳ではなく虚偽表示。** `scripts/check-untranslated.py` が 7 言語すべてを検査する。意図的に原文を残す箇所（i18n キー表、literal に照合するキーワード）は `<!-- allow:source-language:start -->` … `end` で囲み、理由を書く。
...
```bash
gitleaks detect --config .gitleaks.toml --no-git --source .
python3 scripts/check-doc-links.py            # Markdown の相対リンク切れ（CI: doc-links ジョブ）
python3 scripts/check-doc-links.py --selftest  # 検出器が機能していることの確認
# CI が agent-output チェックをミラー: .github/workflows/agent-output-audit.yml
```

**gitignore 対象へのリンクを張らない。** `development/` と `.kiro/` は `.gitignore` にあり、clone した読者には存在しない。ローカル専用のスクリプトや記録に触れる必要があるときは、リンク記法にせず本文で「ローカル専用でリポジトリには含まれない」と書き、リポジトリ内の代替手段（CDK コマンド、`scripts/` 配下のスクリプト、runbook の該当節）を併記する。`doc-links` ジョブがリンク切れで落ちる。
