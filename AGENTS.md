...
- **AgentCore Web Search (UNVERIFIED)**: Created via `AwsCustomResource` (no CFN L1). The Web Search target config/endpoint is NOT yet confirmed against AWS docs — opt-in only (`enableWebSearch=false` default). Override endpoint with `WEB_SEARCH_TARGET_ENDPOINT`. Verify API shape before production
...
| `enableWebSearch` | `false` | AgentCore Web Search tool (requires enableAgentCoreGateway) |
...
| `WEB_SEARCH_TARGET_ENDPOINT` | (placeholder) | Override the Web Search connector endpoint (UNVERIFIED default) |
...
| Web Search target not created | `enableWebSearch=true` without `enableAgentCoreGateway=true` | Web Search requires Gateway; set both flags |
