...
**A**: RAG 検索の権限判定は、各ファイルの権限メタデータ（`.metadata.json`）と、AD から同期した利用者の SID を検索時に突き合わせて行います。**フォルダの NTFS ACL がそのまま自動で反映されるわけではありません。** S3 Access Point を通る要求は 1 つのファイルシステム ID で認可されるため、ファイルごとの ACL は利用者の認可には引き継がれません。したがって権限メタデータの初期投入と、ACL 変更時の更新手順が必要です。詳細は [権限メタデータ変更の整合性モデル](permission-consistency.md) を参照してください。
