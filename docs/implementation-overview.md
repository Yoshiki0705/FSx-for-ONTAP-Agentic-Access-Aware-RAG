...
| ファイル | 役割 |
|---------|------|
| `docker/nextjs/src/app/api/bedrock/kb/retrieve/route.ts` | KB検索API + SIDフィルタリング統合 |
| `lambda/permissions/permission-filter-handler.ts` | 権限フィルタリングLambda（キャッシュ付き） |
...
| 2 | NetworkingStack | ap-northeast-1 | VPC, サブネット, セキュリティグループ |
| 3 | SecurityStack | ap-northeast-1 | Cognito User Pool, Client |
| 4 | StorageStack | ap-northeast-1 | FSx ONTAP + SVM + Volume, S3, DynamoDB×2, AD |
| 5 | AIStack | ap-northeast-1 | Bedrock KB, OpenSearch Serverless |
| 6 | WebAppStack | ap-northeast-1 | Lambda (Docker), CloudFront |
| 7 | EmbeddingStack（任意） | ap-northeast-1 | EC2, ECR |
