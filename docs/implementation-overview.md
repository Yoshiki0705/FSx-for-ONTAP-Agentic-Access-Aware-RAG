...
> 認証モードの選択（メール/パスワード vs AD Federation vs OIDC/LDAP Federation）、ユーザーのSID/UID/GID自動登録、AD/OIDC/LDAP連携設定の詳細は [認証・ユーザー管理ガイド](auth-and-user-management.md) を参照してください。
...
| L5: ユーザー認証 | Cognito JWT / SAML / OIDC | ユーザーレベルの認証・認可 |
| L6: データ認可 | SID / UID+GIDフィルタリング | ドキュメントレベルのアクセス制御 |

### 認証モード

本システムは3つの認証モードを提供し、設定駆動で自動有効化されます。

| モード | 有効化条件 | ユーザー作成 | 権限マッピング |
|--------|----------|------------|-------------|
| メール/パスワード | デフォルト | 管理者が手動作成 | 手動SID登録 |
| SAML AD Federation | `enableAdFederation=true` | 初回サインイン時に自動 | AD Sync Lambda（SID自動取得） |
| OIDC/LDAP Federation | `oidcProviderConfig` 指定 | 初回サインイン時に自動 | Identity Sync Lambda（SID/UID/GID自動取得） |

AD FederationおよびOIDC/LDAP Federationモードでは「ゼロタッチユーザープロビジョニング」を実現し、ファイルサーバーの既存権限がRAGシステムに自動マッピングされます。
