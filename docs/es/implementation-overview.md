# Sistema RAG con Permisos — Descripción General de la Implementación
...
<!-- i18n:abridged -->
> **⚠️ Traducción parcial**: esta traducción cubre solo una parte del original japonés. Consulte la [versión japonesa](../implementation-overview.md) para el contenido completo.
...
Este sistema es un sistema de chatbot RAG (Retrieval-Augmented Generation) que combina Amazon FSx for NetApp ONTAP con Amazon Bedrock, proporcionando filtrado basado en permisos de acceso a archivos (SID). Gestiona la información de ACL NTFS por usuario como metadatos y filtra los resultados de búsqueda en tiempo real, permitiendo la recuperación segura de documentos y la generación de respuestas impulsada por IA.

Toda la infraestructura está definida usando AWS CDK (TypeScript) y puede desplegarse de una vez con `npx cdk deploy --all`.

---

## 1. Aplicación Chatbot — Next.js RAG Chatbot en AWS Lambda

### Detalles de implementación

Una aplicación de chatbot RAG construida con Next.js 15 (App Router) se ejecuta de forma serverless a través de AWS Lambda Web Adapter.
...
#### Diseño del tipo de usuario de S3 Access Point

| Patrón | Tipo de usuario | Origen del usuario | Estilo del volumen | Condición |
|--------|----------------|-------------------|-------------------|-----------|
| A | WINDOWS | Usuario AD existente (Admin) | NTFS/UNIX | SVM unido a AD (recomendado: entornos NTFS) |
| B | WINDOWS | Nuevo usuario AD dedicado | NTFS/UNIX | SVM unido a AD + privilegio mínimo |
| C | UNIX | Usuario UNIX existente (root) | UNIX | Sin unión a AD (recomendado: entornos UNIX) |
| D | UNIX | Nuevo usuario UNIX dedicado | UNIX | Sin unión a AD + privilegio mínimo |

El filtrado SID opera con la misma lógica (basada en metadatos `.metadata.json`) en todos los patrones y no depende del estilo de seguridad del volumen ni del tipo de usuario S3 AP.
...
## 19. Automatización de Operaciones FSx for ONTAP — Lambda + Step Functions
...
| data_preprocessor (FSx for ONTAP S3 AP) | ✅ APROBADO (scan, collect_metadata, generate_tasks) |
...
| `enableAdvancedPermissions` | - | `false` | Control de acceso basado en tiempo + registro de auditoría de decisiones de permisos |
