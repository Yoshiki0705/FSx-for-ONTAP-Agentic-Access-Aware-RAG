...
> **S3 Access Point에 대해**: StorageStack은 FSx ONTAP 볼륨에 대한 S3 Access Point를 자동으로 생성하지만, FlexCache Cache 볼륨에서는 S3 Access Point를 사용할 수 없으므로(2026년 3월 기준), Bedrock KB 데이터 소스로 사용되지 않습니다. 향후 FlexCache 지원이 가능해지면 옵션 C로 활용할 수 있도록 기반이 준비되어 있습니다.
...
```
사용자           Next.js API Route        DynamoDB          Bedrock KB        Bedrock Converse
  │                  │                       │                  │                  │
  │ 1. 쿼리 제출     │                       │                  │                  │
  │─────────────────▶│                       │                  │                  │
  │                  │ 2. 사용자 SID 조회    │                  │                  │
  ...
  │                  │ userSID + groupSIDs   │                  │                  │
  │                  │                       │                  │                  │
  │                  │ 3. Retrieve API (벡터 검색)              │                  │
  │                  │─────────────────────────────────────────▶│                  │
  │                  │◀─────────────────────────────────────────│                  │
  │                  │ 검색 결과 + 메타데이터(allowed_group_sids)                  │
  │                  │                       │                  │                  │
  │                  │ 4. SID 매칭            │                  │                  │
  │                  │ 사용자 SID ∩ 문서 SID                    │                  │
  │                  │ → 매칭: 허용                             │                  │
  │                  │ → 미매칭: 거부                           │                  │
  │                  │                       │                  │                  │
  │                  │ 5. Converse API (허용된 문서만으로 답변 생성)               │
  │                  │────────────────────────────────────────────────────────────▶│
  │                  │◀────────────────────────────────────────────────────────────│
  │                  │                       │                  │                  │
  │ 6. 필터링된      │                       │                  │                  │
