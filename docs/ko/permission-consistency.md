...
**시간 기반 접근 제어(`enableAdvancedPermissions`)의 실패 처리는 두 방향으로 나뉩니다.** 설정 결함(잘못된 타임존, `HH:mm`으로 읽을 수 없는 시각)은 페일 오픈으로, 시간 제한을 해제하고 허용합니다(사용자 잠김 방지). 반면 **시계 자체를 읽을 수 없는 경우에는 거부합니다** — 시간대 내부인지 판단하지 못한 채 허용하면 시간 제한이 조용히 사라지기 때문입니다. 어느 경우에도 SID 대조는 별도로 적용되므로, 이 분기가 권한 인덱스 판정을 약화시키지 않습니다.
