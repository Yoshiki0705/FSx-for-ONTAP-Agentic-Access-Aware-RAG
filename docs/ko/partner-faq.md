...
**A**: RAG 검색의 권한 판정은 각 파일의 권한 메타데이터(`.metadata.json`)와 AD에서 동기화한 사용자 SID를 검색 시점에 대조하여 수행합니다. **폴더의 NTFS ACL이 그대로 자동 반영되는 것은 아닙니다.** S3 Access Point를 통한 요청은 하나의 파일 시스템 ID로 인가되므로, 파일별 ACL은 사용자의 인가로 이어지지 않습니다. 따라서 권한 메타데이터의 초기 투입과 ACL 변경 시의 갱신 절차가 필요합니다. 자세한 내용은 [권한 메타데이터 변경 일관성 모델](permission-consistency.md)을 참조하세요.
