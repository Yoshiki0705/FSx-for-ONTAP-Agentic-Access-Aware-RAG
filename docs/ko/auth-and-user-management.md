...
로그인 화면은 활성화된 인증 방식에 따라 동적으로 버튼을 표시합니다.

| 활성화된 인증 방식 | 로그인 화면 |
|------------------|-----------|
| 이메일/비밀번호만 | 이메일/비밀번호 양식 |
| + SAML | + "AD로 로그인" 버튼 |
| + OIDC | + "{providerName}로 로그인" 버튼 |
| + SAML + OIDC | + 두 버튼 모두 표시 |

> **LDAP 사용자는 어떤 버튼을 선택해야 하나요?**
>
> "{providerName}로 로그인" 버튼을 선택하세요. LDAP는 "인증"이 아닌 "권한 취득"을 담당하는 백엔드 메커니즘입니다. 사용자는 LDAP의 존재를 인식할 필요가 없습니다.
>
> 이메일/비밀번호 양식은 Cognito에 직접 생성된 사용자용입니다. OIDC/LDAP 사용자가 이 양식으로 로그인하면 LDAP 권한 취득 경로가 실행되지 않습니다.
...
![로그인 화면 (SAML + OIDC 하이브리드)](../screenshots/signin-page-saml-oidc-hybrid.png)

![Auth0 OIDC 로그인 페이지](../screenshots/oidc-auth0-login-page.png)

![OIDC 로그인 성공 후 채팅 화면](../screenshots/oidc-auth0-signin-success.png)
