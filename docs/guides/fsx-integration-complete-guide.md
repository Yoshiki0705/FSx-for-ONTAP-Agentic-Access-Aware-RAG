...
## 4. FSx for ONTAPサブネット要件（重要）

### 4.1 デプロイメントタイプ別のサブネット要件

FSx for ONTAPのデプロイメントタイプによって、必要なサブネット数が異なります。

| デプロイメントタイプ | 必要なサブネット数 | 説明 | 推奨用途 |
|-------------------|-----------------|------|---------|
| `SINGLE_AZ_1` | **1つ** | 単一AZにデプロイ | 開発環境、コスト効率重視 |
| `MULTI_AZ_1` | **2つ** | 2つのAZにデプロイ | 本番環境、高可用性重視 |

### 4.2 CDK実装例

```typescript
// lib/modules/storage/constructs/storage-construct.ts
...
const fileSystem = new fsx.CfnFileSystem(this, 'FSxONTAP', {
  ...
  subnetIds: fsxSubnetIds, // ✅ デプロイメントタイプに応じたサブネット数
  ontapConfiguration: {
...
#### エラー: "Exactly 1 subnet IDs are required for SINGLE_AZ_1"

**原因**: `SINGLE_AZ_1`デプロイメントタイプで2つ以上のサブネットを指定している

**解決**:
```typescript
// ❌ 悪い例: 固定値を使用
const fsxSubnetIds = this.props.privateSubnetIds.slice(0, 2);

// ✅ 良い例: デプロイメントタイプに応じて動的に決定
const deploymentType = config.deploymentType || 'SINGLE_AZ_1';
const requiredSubnetCount = deploymentType === 'SINGLE_AZ_1' ? 1 : 2;
const fsxSubnetIds = this.props.privateSubnetIds.slice(0, requiredSubnetCount);
```

#### エラー: "Exactly 2 subnet IDs are required for MULTI_AZ_1"

**原因**: `MULTI_AZ_1`デプロイメントタイプで1つのサブネットしか指定していない

**解決**: 2つの異なるAZのサブネットを指定する

```typescript
// MULTI_AZ_1の場合は2つのサブネットが必要
const fsxSubnetIds = [
  'subnet-0a84a16a1641e970f', // AZ: ap-northeast-1a
  'subnet-0c4599b4863ff4d33'  // AZ: ap-northeast-1c
];
```

### 4.4 デプロイメントタイプの選択ガイド
