...
**A**: RAG 检索的权限判定，是在检索时将每个文件的权限元数据（`.metadata.json`）与从 AD 同步的用户 SID 进行比对。**并非将文件夹的 NTFS ACL 原样自动反映。** 通过 S3 Access Point 的请求以单一文件系统身份进行授权，因此逐文件的 ACL 不会传递到用户的授权。因此需要权限元数据的初始导入，以及 ACL 变更时的更新流程。详情请参见[权限元数据变更一致性模型](permission-consistency.md)。
