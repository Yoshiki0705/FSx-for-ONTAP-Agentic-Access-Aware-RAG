...
> **关于 S3 Access Point**：StorageStack 自动为 FSx ONTAP 卷创建 S3 Access Point，但由于截至 2026 年 3 月 S3 Access Point 不适用于 FlexCache Cache 卷，因此不用作 Bedrock KB 数据源。基础已准备好，以便在 FlexCache 支持可用时作为选项 C 使用。
