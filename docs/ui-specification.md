...
Agentモードでは、プリセットワークフローを選択するか、自由チャットで質問できます。

| ワークフロー | プロンプト | 対象ドキュメント |
|-------------|----------|----------------|
| 📊 財務レポート分析 | 最新の財務レポートを分析して... | confidential/financial-report.md |
| 📝 プロジェクト進捗確認 | プロジェクト計画の進捗状況を... | restricted/project-plan.md |
| 🔍 ドキュメント横断検索 | 社内ドキュメントを横断的に... | 全ドキュメント |
| 📋 人事ポリシー確認 | 人事ポリシーの内容を確認し... | confidential/hr-policy.md |

ワークフロー選択時、プロンプトがチャット入力欄に自動設定されます（`agent-workflow-selected`カスタムイベント）。
...
## 13. Citation Display — アクセスレベルバッジ

### 概要

CitationDisplay コンポーネント（`docker/nextjs/src/components/chat/CitationDisplay.tsx`）は、RAG検索結果の各ソースドキュメントに対してアクセスレベルバッジを表示します。バッジは `.metadata.json` の `access_level` フィールドの値に基づき、日本語ラベルで表示されます。
