# Permission-aware RAG Verification Environment Guide

**🌐 Language:** [日本語](../demo-environment-guide.md) | **English** | [한국어](../ko/demo-environment-guide.md) | [简体中文](../zh-CN/demo-environment-guide.md) | [繁體中文](../zh-TW/demo-environment-guide.md) | [Français](../fr/demo-environment-guide.md) | [Deutsch](../de/demo-environment-guide.md) | [Español](../es/demo-environment-guide.md)
...
| User | Email Address | Password | Role | Permissions |
|---|---|---|---|---|
| Administrator | `admin@example.com` | `DemoAdmin123!` | administrator | Can view all documents |
| General User | `user@example.com` | `DemoUser123!` | user | public documents only |

Authentication is managed by Amazon Cognito.
...
1. Access the CloudFront URL → `/ja/signin`
2. Sign in with a test user
3. **KB Mode**: Select a model on the chat screen → Verify permission filtering in RAG search
4. **Agent Mode**: Click the "🤖 Agent" button in the header → Select an Agent → Choose a workflow or free chat

### Verifying Permission Differences

When the administrator and general user ask the same question, SID filtering returns different results.
The same permission controls apply in both KB Mode and Agent Mode.
