...
ボタンの表示名は `oidcProviderConfig.providerName` の値で決まります:

| `providerName` 設定値 | ボタン表示 |
|----------------------|-----------|
| `Auth0` | 「Auth0でサインイン」 |
| `Keycloak` | 「Keycloakでサインイン」 |
| `Okta` | 「Oktaでサインイン」 |
| `EntraID` | 「EntraIDでサインイン」 |

> **LDAPユーザーはどのボタンを選ぶべきか？**
>
> 「{providerName}でサインイン」ボタンを選んでください。LDAPは「認証」ではなく「権限取得」を担当するバックエンドの仕組みです。ユーザーがLDAPの存在を意識する必要はありません。
>
> ```
> 認証（ユーザーが操作）:  OIDC IdP（Auth0/Keycloak/Okta等）でサインイン
>                              ↓
> 権限取得（自動・透過的）:  Identity Sync Lambda → LDAP Connector → OpenLDAP
>                              ↓
> 結果:                      DynamoDB に uid/gid/グループ が自動保存
> ```
>
> メール/パスワードフォームはCognitoに直接作成されたユーザー用です。OIDC/LDAPユーザーがこちらでサインインしても、LDAP権限取得パスは実行されません。
