...
AD Federationが有効な場合のサインイン画面:

![サインイン画面（AD Federation有効）](../docs/screenshots/signin-page-with-ad-federation.png)
...
OIDC/LDAP Federation機能追加後のCDK synth検証結果:

- CDK synth: ✅ 成功（CloudFormationテンプレート正常生成）
- 既存スタック状態: ✅ 全6スタック正常（UPDATE_COMPLETE / CREATE_COMPLETE）
- アプリケーション応答: ✅ CloudFront URL HTTP 200（38KB）
- 後方互換性: ✅ 既存SAML AD Federationサインイン画面正常表示
- ユニットテスト: ✅ 130テスト全パス
- プロパティテスト: ✅ 52テスト全パス（17プロパティ、numRuns=20）

### OIDC Federation検証（未実施）

OIDC IdP（Keycloak/Okta/Entra ID）のセットアップ後に以下を検証予定:

- [ ] OIDC IdP登録（Cognito UserPoolIdentityProviderOidc）
- [ ] サインイン画面にOIDCボタン表示
- [ ] OIDC認証フロー（Cognito Hosted UI → IdP → コールバック）
- [ ] Identity Sync Lambda（OIDC Claims / LDAP権限取得）
- [ ] DynamoDB拡張スキーマ（uid/gid/unixGroups/oidcGroups）
- [ ] Permission Resolver（UID/GID / Hybrid戦略）
