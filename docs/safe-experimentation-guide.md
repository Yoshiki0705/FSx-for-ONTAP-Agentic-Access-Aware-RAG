...
┌─────────────────────────────────────────────────────────────────────┐
│                                                                       │
│  🟢 GREEN ZONE（自由に試行錯誤可能）                                   │
│  ┌─────────────────────────────────────────────────────────────┐     │
│  │ • デモデータでの検索テスト                                      │     │
│  │ • ユーザー切替による権限確認                                    │     │
│  │ • Agent モードの試行・Agent 作成                               │     │
│  │ • UI カスタマイズ（Next.js ソース変更）                         │     │
│  │ • Smart Routing ON/OFF、モデル選択変更                         │     │
│  │ • CDK パラメータ変更 → 再デプロイ                              │     │
│  │ • 新しいデモドキュメントの追加                                  │     │
│  │ • Guardrails ポリシーの調整                                    │     │
│  └─────────────────────────────────────────────────────────────┘     │
│                                                                       │
│  🟡 YELLOW ZONE（チェックリスト確認後に実施）                          │
│  ┌─────────────────────────────────────────────────────────────┐     │
│  │ • 実データの投入（→ データレディネス評価テンプレート記入必須）     │     │
│  │ • 顧客 IdP（AD/OIDC/LDAP）への接続                            │     │
│  │ • 音声チャットの有効化（コスト発生: ~$70-100/月）               │     │
│  │ • VPC エンドポイントの追加                                     │     │
│  │ • Transfer Family の有効化（パートナーアクセス開始）             │     │
│  │ • KMS 暗号化の有効化                                          │     │
│  └─────────────────────────────────────────────────────────────┘     │
│                                                                       │
│  🔴 RED ZONE（承認必須 — 単独で実施しない）                            │
│  ┌─────────────────────────────────────────────────────────────┐     │
│  │ • 本番 AD への直接接続                                         │     │
│  │ • PII 未分類データの投入                                       │     │
│  │ • 監査ログ未設定での機密データ利用                              │     │
│  │ • 公開インターネットからの無制限アクセス許可                     │     │
│  │ • 本番アカウントでの PoC 実施                                  │     │
│  │ • Guardrails 無効での機密データ利用                            │     │
│  │ • 暗号化なしでの機密データ保存                                 │     │
│  └─────────────────────────────────────────────────────────────┘     │
│                                                                       │
└─────────────────────────────────────────────────────────────────────┘
...
判断基準:
  🟢 → そのまま実行OK
  🟡 → データレディネス評価 or チェックリスト確認後に実行
  🔴 → ビジネススポンサー + セキュリティ担当の承認後に実行
```
...
| AWS ドキュメント（FSx for ONTAP） | https://docs.aws.amazon.com/fsx/latest/ONTAPGuide/ |
