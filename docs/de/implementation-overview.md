# Permission-Aware RAG System — Implementierungsübersicht
...
<!-- i18n:abridged -->
> **⚠️ Gekürzte Übersetzung**: Diese Übersetzung deckt nur einen Teil des japanischen Originals ab. Der vollständige Inhalt steht in der [japanischen Version](../implementation-overview.md).
...
Dieses System ist ein RAG (Retrieval-Augmented Generation) Chatbot-System, das Amazon FSx for NetApp ONTAP mit Amazon Bedrock kombiniert und eine dateizugriffsberechtigungsbasierte (SID) Filterung bietet. Es verwaltet NTFS ACL-Informationen pro Benutzer als Metadaten und filtert Suchergebnisse in Echtzeit, was eine sichere Dokumentensuche und KI-gestützte Antwortgenerierung ermöglicht.

Die gesamte Infrastruktur ist mit AWS CDK (TypeScript) definiert und kann auf einmal mit `npx cdk deploy --all` bereitgestellt werden.

---

## 1. Chatbot-Anwendung — Next.js RAG Chatbot auf AWS Lambda

### Implementierungsdetails

Eine mit Next.js 15 (App Router) erstellte RAG-Chatbot-Anwendung wird serverlos über AWS Lambda Web Adapter ausgeführt.
...
| Schicht | Technologie |
|---------|------------|
| Framework | Next.js 15 (App Router, Standalone-Ausgabe) |
| UI | React 18 + Tailwind CSS |
| Authentifizierung | Amazon Cognito (JWT) |
| KI/RAG | Amazon Bedrock Knowledge Base Retrieve API + Converse API |
| Laufzeit | Lambda Web Adapter (Rust) + Docker Container |
| CDN | Amazon CloudFront |

### Hauptfunktionen

- **RAG-Suche**: Führt Vektorsuche über Bedrock Knowledge Base durch und generiert Antworten unter Bezugnahme auf relevante Dokumente
- **SID-Filterung**: Filtert Suchergebnisse basierend auf Benutzer-SID-Informationen (Details in Abschnitt 7)
- **KB/Agent-Modus-Umschaltung**: Wechsel zwischen KB-Modus (Dokumentensuche) und Agent-Modus (mehrstufiges Reasoning) über Header-Toggle
- **Kartenbasierte aufgabenorientierte UI**: Vor dem Start eines Chats zeigt der KB-Modus zweckspezifische Karten (Dokumentensuche, Zusammenfassungserstellung, Quiz-Generierung usw.) und der Agent-Modus Workflow-Karten (Finanzanalyse, Projektmanagement usw.) in einem Rasterlayout an. Unterstützt Favoritenverwaltung und Kategoriefilterung
- **Agent-Modus (InvokeAgent API)**: Erreicht mehrstufiges Reasoning mit SID-Filterung durch Bedrock Agent + Permission-aware Action Group. Fällt auf KB-Hybridmodus zurück, wenn der Agent-Aufruf fehlschlägt
- **Mehrsprachige Unterstützung**: 8 Sprachen — Japanisch, Englisch, Koreanisch, Chinesisch (vereinfacht/traditionell), Französisch, Deutsch und Spanisch
- **Zitat-Anzeige**: Zeigt Quellinformationen der als Grundlage für Antworten verwendeten Dokumente
- **Cognito-Authentifizierung**: Anmeldung/Abmeldung und Sitzungsverwaltung
...
| Muster | Benutzertyp | Benutzerquelle | Volume-Style | Bedingung |
|--------|------------|----------------|-------------|-----------|
| A | WINDOWS | Vorhandener AD-Benutzer (Admin) | NTFS/UNIX | AD-beigetretene SVM (empfohlen: NTFS-Umgebungen) |
| B | WINDOWS | Neuer dedizierter AD-Benutzer | NTFS/UNIX | AD-beigetretene SVM + minimale Berechtigung |
| C | UNIX | Vorhandener UNIX-Benutzer (root) | UNIX | Nicht AD-beigetreten (empfohlen: UNIX-Umgebungen) |
| D | UNIX | Neuer dedizierter UNIX-Benutzer | UNIX | Nicht AD-beigetreten + minimale Berechtigung |

Die SID-Filterung arbeitet bei allen Mustern mit derselben Logik (`.metadata.json`-metadatenbasiert) und ist unabhängig vom Sicherheitsstil des Volumes oder dem S3 AP-Benutzertyp.
...
- High-performance model: `anthropic.claude-sonnet-4-6`
...
|-----------|----------|-------|
| `DRY_RUN` | `true` | Sicherer Standard — protokolliert ohne Ausführung |
| `MAX_GROW_PER_ACTION_PCT` | 50% | Max. Wachstumsrate pro Aktion |
| `MAX_GROW_PER_DAY_GIB` | 500 GiB | Max. tägliche Gesamterweiterung |

### TLS-Verifizierung

TLS-Zertifikatverifizierung ist standardmäßig aktiviert. Für Produktion: CA-Bundle über `ONTAP_CA_CERT_PATH`. Für Lab/PoC: `ONTAP_VERIFY_SSL=false`.

### Tests

38 Unit-Tests für TLS-Verifizierung, SnapMirror-Dual-Initialisierung (10 Aktionen), Kapazitäts-Schutzmaßnahmen und S3 AP-Operationen.
