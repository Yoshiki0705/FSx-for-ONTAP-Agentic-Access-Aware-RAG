...
**時間ベースアクセス制御（`enableAdvancedPermissions`）の失敗時の扱いは 2 通りに分かれます。** 設定の不備（無効なタイムゾーン、`HH:mm` として読めない時刻）はフェイルオープンで、時間制限を外して許可します（利用者のロックアウトを避けるため）。一方で**時計そのものが読めない場合は拒否します** — 時間帯の内か外かを判定できないまま許可すると、時間制限が黙って外れるためです。いずれの場合も SID による突き合わせは別に適用され、この分岐が権限索引の判定を弱めることはありません。
