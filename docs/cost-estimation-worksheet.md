...
> **Smart Routing効果に関する注記**: 上記の60/30/10%分布はデモ環境での観測値に基づく想定です。実際の分布はクエリパターン（業種、ユーザー層、利用目的）に依存します。本番環境での分布は `SmartRouting/RoutingTier` CloudWatchメトリクスで実測してください。

#### コスト最適化オプション（計画中）

以下は 2026 Q2 AI Update で導入予定のコスト最適化機能です。

| 最適化手法 | 削減効果（見込み） | 適用条件 | ステータス |
|-----------|-----------------|---------|-----------|
| **Prompt Caching** | 入力トークン 30-90% 削減 | システムプロンプト ≥ 2,048文字、同一ユーザー 5分以内の連続クエリ | 🔜 Phase 1 |
| **Model Distillation** | Lightweight tier コスト 70-90% 削減 | 蒸留モデルのPermission判定精度 ≥ 95% | 🔜 Phase 5 |
| **Batch Inference** | 非リアルタイム処理 50% 削減 | KB Auto-Sync メタデータ生成のみ | 🔜 Phase 1 |

**Prompt Caching適用後の試算例（100 queries/日）:**
```
Prompt Caching なし: $42/月
Prompt Caching あり（キャッシュヒット率 70% 想定）:
  - キャッシュヒット時の入力トークンコスト: 90% 削減
  - 出力トークンコストは変わらない
  - 加重平均削減率: 入力トークンが総コストの約60% → 60% × 70% × 90% = 約38%削減
  → 推定月額: $42 × (1 - 0.38) ≈ $26/月（約38%削減）
```

> **実測値（2026-06-08 検証環境）:**
> - System Prompt: **1161 tokens** がキャッシュされ、2回目以降の呼び出しで cache hit 確認
> - Cache hit 率: 1161 / (1161 + 2137) = **33%** of total input tokens cached
> - **重要**: Prompt Caching は **Messages API (InvokeModel) でのみ動作**。Converse API は `cacheControl` を無視する
> - 実効コスト削減: cache hit 時の input token は通常の 1/10 コスト → 33% × 90% = **約30% 入力トークンコスト削減**

> **注**: Prompt Cachingの効果はユーザーのセッション継続性に大きく依存します。
> - **70%ヒット率の前提**: インタラクティブチャットセッション（1ユーザーが5分以内に3-5クエリを連続送信）を想定
> - **ヒット率が高くなるケース**: 業務時間中の集中利用、ドキュメント深掘り型の連続質問
