...
> **⚠️ 重要**: 本表は技術的な対応状況の参考情報であり、法的・コンプライアンス上の適合性を保証するものではありません。規制対象ワークロードでの利用には、必ず顧客固有の法務・コンプライアンス評価を実施してください。

### 個人情報保護法（日本）

| 要件 | 本システムの技術的対応 | 顧客責任範囲 |
|------|---------------------|-------------|
| 利用目的の特定・通知 | — | 利用規約・プライバシーポリシーの策定 |
| 安全管理措置 | KMS暗号化、VPCエンドポイント、WAF、6層防御 | 組織的・人的安全管理措置の実施 |
| アクセス制御 | SID/UID/GIDフィルタリング、Fail-Closed設計 | アクセス権限の適切な設定・棚卸し |
| 委託先管理 | IAM最小権限、CloudTrail監査ログ | 委託契約の締結、監督体制の構築 |
| 漏えい報告 | CloudWatch Alarm、SNS通知 | 報告体制の構築、個人情報保護委員会への報告 |
| 開示・訂正・削除請求 | — | 請求対応プロセスの構築 |

### 医療情報（3省2ガイドライン準拠の観点）

| 要件 | 本システムの技術的対応 | 顧客責任範囲 |
|------|---------------------|-------------|
| 情報の機密性確保 | 権限フィルタリング、暗号化、VPC分離 | 医療情報の分類、アクセス権限設計 |
| 情報の完全性確保 | DynamoDB監査ログ、CloudTrail | 改ざん検知体制の構築 |
| 情報の可用性確保 | FSx自動バックアップ、Multi-AZ（オプション） | DR計画の策定、RTO/RPO定義 |
| 監査証跡 | 構造化監査ログ（JSON）、Athena分析 | 監査体制の構築、定期レビュー |
| 外部保存 | AWS東京リージョン（国内） | 外部保存に関する院内規程の策定 |

### 金融（FISC安全対策基準の観点）

| 要件 | 本システムの技術的対応 | 顧客責任範囲 |
|------|---------------------|-------------|
| アクセス管理 | IAM + Cognito + SIDフィルタリング | アクセス管理規程の策定 |
| 暗号化 | KMS CMK、TLS 1.2+ | 暗号鍵管理規程の策定 |
| 監査ログ | CloudTrail + DynamoDB監査テーブル | ログ保管期間の決定（7年等） |
| 障害対策 | CloudWatch Alarm、自動復旧 | BCP/DR計画の策定 |
| 委託管理 | AWS責任共有モデル | 委託先管理規程の策定 |

### 公共セクター（政府情報システムのセキュリティ要件）

| 要件 | 本システムの技術的対応 | 顧客責任範囲 |
|------|---------------------|-------------|
| 情報の格付け | メタデータによる分類（public/confidential/restricted） | 情報格付け基準の策定 |
| アクセス制御 | 部局×職位の権限フィルタリング | アクセス権限の設計・承認 |
| 監査 | 構造化ログ、Athena分析 | 監査計画の策定、定期監査の実施 |
| データ所在 | AWS東京リージョン | データ所在要件の確認 |
| AI利用 | Guardrails、Responsible AI Statement | AI利用ガイドラインの策定 |
