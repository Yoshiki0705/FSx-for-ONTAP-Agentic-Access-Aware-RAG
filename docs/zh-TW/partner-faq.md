...
**A**: RAG 搜尋的權限判定，是在搜尋時將每份檔案的權限中繼資料（`.metadata.json`）與從 AD 同步的使用者 SID 進行比對。**並非將資料夾的 NTFS ACL 原樣自動反映。** 透過 S3 Access Point 的請求以單一檔案系統身分進行授權，因此逐檔案的 ACL 不會傳遞至使用者的授權。因此需要權限中繼資料的初始匯入，以及 ACL 變更時的更新程序。詳情請參見[權限中繼資料變更一致性模型](permission-consistency.md)。
