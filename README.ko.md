...
| 14 | OIDC/LDAP Federation + ONTAP Name-Mapping | OIDC IdP(Auth0/Keycloak/Okta) 연동, LDAP 직접 쿼리(OpenLDAP/FreeIPA)를 통한 UID/GID 자동 취득, ONTAP REST API name-mapping(UNIX→Windows 사용자 매핑). 설정 기반 자동 활성화. `oidcProviderConfig` + `ldapConfig` + `ontapNameMappingEnabled`로 활성화. **Phase 2 확장**: 멀티 OIDC IdP(`oidcProviders` 배열), OIDC 그룹 기반 문서 접근 제어(`allowed_oidc_groups`), LDAP TLS 인증서 검증(`tlsCaCertArn`), 토큰 리프레시/세션 관리, Fail-Closed 모드(`authFailureMode`), LDAP 헬스 체크(EventBridge + CloudWatch Alarm), 인증 감사 로그(`auditLogEnabled`) | SecurityStack |
| 15 | Agent Registry 통합 | Agent Directory에 AWS Agent Registry(AgentCore) 탭 추가. 조직 내 Agent, 도구, MCP 서버를 시맨틱 검색, 가져오기, 게시. 승인 워크플로 지원. 크로스 리전 액세스(`agentRegistryRegion`). `enableAgentRegistry=true`로 활성화 | AIStack, WebAppStack |
| 16 | 멀티모달 RAG 검색 | Amazon Nova Multimodal Embeddings를 사용한 텍스트, 이미지, 동영상, 오디오 크로스모달 검색. Embedding Model Registry + KB Config Strategy 패턴으로 확장성 확보. Dual KB 아키텍처(텍스트 전용 + 멀티모달 병렬 운용) 지원. `embeddingModel: "nova-multimodal"`로 옵트인 활성화 | AIStack |
...
| 기능 | API 상태 | 구현 방식 | 참고사항 |
|------|---------|---------|---------|
| Agent Registry | Preview (2026년 4월) | SigV4 서명 HTTP + REST 경로 매핑 | SDK 명령(`search_registry_records` 등)은 boto3/CLI에서 사용 가능. Node.js는 SigV4 HTTP 사용. 먼저 `create_registry`로 레지스트리 생성 필요. `agentRegistryArn`으로 ARN 지정 |
| 멀티모달 RAG | GA (2025년 11월) | AWS SDK v3 (BedrockAgentRuntimeClient) | 완전 구현. Nova Multimodal Embeddings는 us-east-1, us-west-2에서만 지원 |
| Guardrails Org Safeguards | GA (2026년 4월) | AWS SDK v3 (BedrockClient) | 완전 구현. Organizational Safeguards는 AWS Organizations 관리 계정에서 설정 |
| AgentCore Episodic Memory | GA (2025년 12월) | AWS SDK v3 (BedrockAgentCoreClient) | `episodicMemoryStrategy`에 `reflectionConfiguration.namespaces` 파라미터 필수. 미지정 시 CreateMemory에서 "Invalid memory strategy input" 오류 반환 |
| Nova Sonic 음성 채팅 | GA (2025년 12월) | REST + Bedrock Converse API | Phase 1 구현 (REST 기반). 실시간 양방향 스트리밍에는 API Gateway WebSocket (Phase 2) 필요 |
| AgentCore Policy | GA (2026년 3월) | SigV4 서명 HTTP (Policy Engine + Gateway 모델) | GA 버전에서 Policy Engine + Gateway 아키텍처로 변경. 정책은 Cedar 언어로 작성. IAM 액션이 `bedrock-agentcore:CreatePolicyEngine` 등으로 업데이트 |

**음성 채팅 Phase 1 / Phase 2 아키텍처:**

| 항목 | Phase 1 (현재) | Phase 2 (향후) |
|------|---------------|---------------|
| 통신 방식 | REST (POST /api/voice/stream) | WebSocket (API Gateway) |
| 음성 처리 | Bedrock Converse API로 음성→텍스트 변환 후 RAG 파이프라인에 입력 | Nova Sonic InvokeModelWithBidirectionalStream으로 양방향 스트리밍 |
| 지연 시간 | 중간 (버퍼링 + 일괄 처리) | 낮음 (실시간 스트리밍) |
| 권한 필터링 | ✅ 기존 SID/UID/GID 필터링 적용 | ✅ 동일 로직 적용 |

**v4.0.0 기능 플래그:**

v4.0.0 신규 기능(Guardrails 관리 패널, Agent Registry 탭, AgentCore Policy 섹션 등)의 UI 표시는 런타임에 Lambda 환경 변수에서 플래그를 가져오는 기능 플래그 API(`/api/config/features`)로 제어됩니다. CDK 파라미터를 통해 기능을 활성화하면 해당 Lambda 환경 변수가 설정되어 프론트엔드에 자동으로 반영됩니다.

**멀티 에이전트 협업:**

`enableAgent=true`일 때 `enableMultiAgent`는 기본적으로 활성화됩니다(명시적으로 `false`로 설정한 경우에만 비활성화). Bedrock Agents는 대기 비용이 없으므로 멀티 에이전트 모드를 활성화해도 추가 실행 비용이 발생하지 않습니다. 토큰 소비는 실제로 멀티 에이전트 모드에서 채팅할 때만 3~6배 증가합니다.

**npm 의존성 (v4.0.0에서 추가):**

`docker/nextjs/package.json`에 다음 패키지가 필요합니다 (SigV4 서명에 사용):
```bash
cd docker/nextjs
...
![Agent Directory](docs/screenshots/v4-agent-directory-registry-ko.png)
...
| L6: 데이터 인가 | SID / UID+GID / OIDC 그룹 필터링 | 문서 수준 접근 제어. Fail-Closed 모드(`authFailureMode=fail-closed`)로 권한 취득 실패 시 로그인 차단 가능 |
...
### ⚠️ 기존 AWS 환경에 대한 영향에 관한 중요 안내

이 CDK 프로젝트는 다음 AWS 리소스를 생성 및 변경합니다. **기존 프로덕션 환경이나 팀 공유 환경에 배포하는 경우, 사전에 영향을 확인하세요.**

#### 이 시스템의 인증과 AWS 관리 콘솔 인증은 완전히 독립적입니다

RAG 시스템의 인증 방식(OIDC / LDAP / 이메일-비밀번호)과 AWS 관리 콘솔 접근(IAM Identity Center / IAM 사용자)은 **완전히 독립적인 시스템**입니다. RAG 시스템의 인증 방식을 변경해도 관리 콘솔 접근에는 영향이 없습니다.
...
#### Managed AD에 대한 참고 사항

`adPassword`를 설정하면 AWS Managed Microsoft AD가 생성되지만, **AD를 생성하는 것만으로는 Identity Center에 영향을 주지 않습니다.**

문제는 다음과 같은 **수동 조작**을 수행한 경우에만 발생합니다:

1. CDK가 Managed AD를 생성 (`adPassword` 설정)
2. **수동으로** Identity Center의 ID 소스를 "Identity Center 디렉터리"에서 "Active Directory"로 변경
3. Identity Center가 Managed AD 사용자를 ID 소스로 사용하기 시작
4. `cdk destroy`로 Managed AD 삭제
5. → Identity Center의 ID 소스가 손실되어 콘솔 접근이 차단됨

**완화 조치:**
- Identity Center의 ID 소스를 "Identity Center 디렉터리"(기본값)로 유지 — 변경하지 마세요
- 백업으로 IAM 사용자 콘솔 접근을 유지하세요
- `cdk destroy` 실행 전에 Identity Center ID 소스 설정을 확인하세요

**확인 명령어:**
```bash
# IAM Identity Center 인스턴스 및 사용자 확인
...
> **⚠️ 중요**: `adPassword`를 설정하면 AWS Managed Microsoft AD가 생성됩니다. AD를 생성하는 것만으로는 Identity Center에 영향을 주지 않습니다. 그러나 수동으로 Identity Center의 ID 소스를 Managed AD로 변경한 경우, `cdk destroy`로 AD를 삭제하면 Identity Center 사용자가 손실됩니다. Identity Center의 ID 소스를 "Identity Center 디렉터리"(기본값)로 유지하세요.
...
| OIDC 로그인 `invalid_request` | issuerUrl 불일치 | `oidcProviderConfig.issuerUrl`이 IdP의 `/.well-known/openid-configuration`의 `issuer` 필드와 정확히 일치하는지 확인. Auth0는 후행 슬래시 필요(`https://xxx.auth0.com/`) |
| OIDC 로그인 `Attribute cannot be updated` | Cognito User Pool의 email 속성이 `mutable: false` | CDK의 `standardAttributes.email.mutable`이 `true`인지 확인. `mutable`은 User Pool 생성 후 변경 불가하므로 User Pool 재생성 필요 |
| OIDC IdP 수동 삭제 후 CDK 배포 실패 | CDK 스택 상태 불일치 | Cognito IdP를 수동으로 삭제·재생성하면 CDK 스택 상태와 실제 상태가 불일치. CDK 배포로만 관리하고 수동 조작은 피할 것 |
| LDAP 헬스체크 Lambda 타임아웃 | VPC Lambda가 Secrets Manager에 접근 불가 | NAT Gateway 또는 `enableVpcEndpoints=true` 필요. 테스트: `aws lambda invoke --function-name perm-rag-demo-demo-ldap-health-check /tmp/result.json` |
| LDAP 헬스체크 Alarm이 ALARM 상태 | LDAP 연결 실패 | CloudWatch Logs에서 구조화된 로그 확인. Connection error → SG/VPC, Bind error → 비밀번호/DN, Search error → baseDN |
| `--exclusively` 없이 Networking 스택 업데이트 실패 | VPC CrossStack Export 의존성 | `npx cdk deploy perm-rag-demo-demo-Security perm-rag-demo-demo-WebApp --exclusively` 사용 |
| 이메일/비밀번호 로그인 시 `AD_EC2_INSTANCE_ID is required` 오류 | AD 미설정 시 SID 동기화가 실행됨 | v3.5.0에서 수정됨. `AD_TYPE` 기본값을 `none`으로 변경, AD 미설정 시 SID 동기화 건너뜀. 이전 Lambda 사용 시 재배포 필요 |
| Fail-Closed 모드가 발동하지 않음 (LDAP 사용자 미검출 시) | 설계대로의 동작 | LDAP 사용자 미검출은 오류가 아닌 OIDC 클레임으로의 폴백. Fail-Closed는 LDAP 연결 타임아웃, Secrets Manager 취득 실패 등 치명적 오류 시에만 발동 |
| `oidcProviderConfig`→`oidcProviders` 마이그레이션 시 CDK 배포 실패 | Cognito IdP 리소스 ID 충돌 | v3.5.0에서 수정됨. 첫 번째 IdP의 CDK 리소스 ID를 `OidcIdP`로 고정하여 마이그레이션 호환성 유지. 이전 버전의 경우 Security 스택 `cdk destroy` → 재배포 필요 |
| ONTAP REST API `User is not authorized` | fsxadmin 패스워드 미설정 | `aws fsx update-file-system --file-system-id <FS_ID> --ontap-configuration '{"FsxAdminPassword":"<PASSWORD>"}'`로 설정. Secrets Manager에 저장하고 `ontapAdminSecretArn`에 지정 |
...
기존 환경에 맞는 인증 방식을 선택하세요. **어떤 방식을 선택하든 AWS 관리 콘솔 접근에는 영향이 없습니다.**

| 기존 환경 | 권장 방식 | 로그인 버튼 | 설정 |
|-----------|----------|------------|------|
| 특별한 것 없음 (시험해보고 싶을 때) | 이메일/비밀번호 | 폼만 | 설정 불필요 (기본값) |
| Okta 사용 중 | OIDC Federation | "Okta로 로그인" | `oidcProviderConfig.providerName: "Okta"` |
| Keycloak 사용 중 | OIDC + LDAP | "Keycloak로 로그인" | `oidcProviderConfig` + `ldapConfig` |
| Entra ID (Azure AD) 사용 중 | OIDC Federation | "EntraID로 로그인" | `oidcProviderConfig.providerName: "EntraID"` |
| Auth0 사용 중 | OIDC Federation | "Auth0로 로그인" | `oidcProviderConfig.providerName: "Auth0"` |
| Windows AD 사용 중 | SAML AD Federation | "AD로 로그인" | `enableAdFederation: true` |
| 여러 IdP 동시 사용 | Multi-OIDC | 각 IdP별 버튼 | `oidcProviders` 배열 |

> **버튼 이름은 커스터마이즈 가능**: `providerName`에 설정한 이름이 버튼에 그대로 표시됩니다. 예를 들어 `"Corporate SSO"`로 설정하면 "Corporate SSO로 로그인"이 표시됩니다.

각 인증 방식은 "설정 기반 자동 활성화"로 동작합니다. `cdk.context.json`에 설정값을 추가하기만 하면 활성화되며, 추가 AWS 리소스 비용은 거의 없습니다. SAML + OIDC 동시 활성화도 가능합니다.

자세한 내용은 [인증 및 사용자 관리 가이드](docs/en/auth-and-user-management.md)를 참조하세요.
...
> **⚠️ issuerUrl 주의**: `oidcProviderConfig.issuerUrl`은 IdP의 `/.well-known/openid-configuration`의 `issuer` 필드 값과 정확히 일치해야 합니다. Auth0는 후행 슬래시 필요(`https://xxx.auth0.com/`), Keycloak은 불필요(`https://keycloak.example.com/realms/main`). 불일치 시 Cognito 토큰 검증에서 `invalid_request` 오류가 발생합니다.

> **⚠️ OIDC 페더레이션 2단계 배포**: OIDC 구성에서는 OAuth 콜백에 `cloudFrontUrl`이 필요하지만, 첫 번째 배포 시에는 CloudFront URL을 알 수 없습니다. 다음 단계를 따르세요:
> 1. `cloudFrontUrl` 없이 `cdk deploy` 실행
> 2. WebApp 스택 출력에서 CloudFront URL 가져오기: `aws cloudformation describe-stacks --stack-name perm-rag-demo-demo-WebApp --query 'Stacks[0].Outputs[?OutputKey==\`CloudFrontUrl\`].OutputValue' --output text`
> 3. `cdk.context.json`에 `cloudFrontUrl`을 추가하고 재배포
> 4. OIDC IdP의 Allowed Callback URLs에 `https://{cognito-domain}.auth.{region}.amazoncognito.com/oauth2/idpresponse`를 설정

##### Phase 2 확장 기능

Phase 2에서는 다음 7가지 확장 기능이 추가되었습니다. 모두 `cdk.context.json` 파라미터로 제어합니다.

**멀티 OIDC IdP 구성 (`oidcProviders` 배열):**

`oidcProviderConfig`(단일 IdP) 외에 `oidcProviders` 배열로 여러 OIDC IdP를 동시에 등록할 수 있습니다. 로그인 화면에 각 IdP의 버튼이 동적으로 표시됩니다. `oidcProviderConfig`와 `oidcProviders`는 상호 배타적 설정입니다.
...
**Fail-Closed 모드 (`authFailureMode`):**

기본값은 Fail-Open(권한 취득 실패 시에도 로그인 계속). 고보안 환경에서는 `authFailureMode: "fail-closed"`를 설정하면 권한 취득 실패 시 로그인 자체를 차단합니다.

> **⚠️ Fail-Closed 발동 조건**: LDAP 사용자가 발견되지 않는 경우는 오류가 아니며, OIDC 클레임만으로 폴백합니다（로그인 계속）. Fail-Closed는 LDAP 연결 타임아웃, Secrets Manager 패스워드 취득 실패, DynamoDB 쓰기 실패 등 치명적 오류 시에만 발동합니다.

**LDAP 헬스 체크 (`healthCheckEnabled`):**

`ldapConfig` 지정 시 자동 활성화(기본값: `true`). EventBridge Rule로 5분 간격의 정기 실행을 수행하여 LDAP 연결·바인드·검색의 정상성을 검증합니다. 실패 시 CloudWatch Alarm으로 알림합니다.

> **⚠️ VPC 네트워크 요건**: 헬스체크 Lambda는 VPC 내에 배치되므로, Secrets Manager(바인드 패스워드 취득)에 접근하려면 NAT Gateway 또는 Secrets Manager VPC 엔드포인트가 필요합니다. `enableVpcEndpoints=true` 사용을 권장합니다.

> **검증 방법**: LDAP 헬스체크가 정상 동작하는지 확인하려면:
> ```bash
...
**감사 로그 (`auditLogEnabled`):**

`auditLogEnabled: true`로 인증 이벤트(로그인 성공/실패)를 DynamoDB 감사 테이블에 기록합니다. TTL에 의한 자동 삭제(기본값: 90일). 감사 테이블 쓰기 실패 시에도 로그인은 차단되지 않습니다.

**TLS 인증서 검증 (`tlsCaCertArn`, `tlsRejectUnauthorized`):**

`ldapConfig` 내에 `tlsCaCertArn`(Secrets Manager의 CA 인증서 ARN)과 `tlsRejectUnauthorized`(기본값: `true`)를 지정하여 LDAPS 연결의 커스텀 CA 인증서 검증을 제어합니다. 개발 환경에서는 `tlsRejectUnauthorized: false`로 자체 서명 인증서를 허용할 수 있습니다.

**OIDC 그룹 기반 문서 접근 제어:**

문서 메타데이터에 `allowed_oidc_groups`를 설정하면 사용자의 `oidcGroups`와의 교차 검사로 접근 제어를 수행합니다. SID/UID-GID 매칭 실패 시의 폴백으로도 기능합니다.

**토큰 리프레시 및 세션 관리:**

OIDC 인증 후 액세스 토큰을 자동 리프레시합니다. 만료 5분 전에 백그라운드에서 리프레시를 실행하고, 리프레시 토큰 만료 시 로그인 화면으로 리다이렉트합니다.

**Phase 2 CDK 파라미터:**

| 파라미터 | 타입 | 기본값 | 설명 |
|---------|------|--------|------|
| `oidcProviders` | array | (없음) | 복수 OIDC IdP 설정 배열 (`oidcProviderConfig`와 상호 배타) |
| `authFailureMode` | string | `fail-open` | 권한 취득 실패 시 동작 (`fail-open` / `fail-closed`) |
| `auditLogEnabled` | boolean | `false` | 인증 감사 로그 DynamoDB 테이블 생성 |
| `auditLogRetentionDays` | number | `90` | 감사 로그 보존 일수 (TTL 자동 삭제) |
| `healthCheckEnabled` | boolean | `true` | LDAP 헬스 체크 Lambda + EventBridge + CloudWatch Alarm |
| `tlsCaCertArn` | string | (없음) | `ldapConfig` 내: LDAPS용 커스텀 CA 인증서의 Secrets Manager ARN |
| `tlsRejectUnauthorized` | boolean | `true` | `ldapConfig` 내: TLS 인증서 검증 (`false`로 자체 서명 인증서 허용) |
...
| `guardrailsConfig` | *(없음)* | Guardrails 상세 설정(`contentFilters`, `topicPolicies`, `piiConfig`, `contextualGrounding`). `enableGuardrails=true` 시에만 유효. 미설정 시 기본 구성(전체 카테고리 HIGH) |
...
| `enableEpisodicMemory` | `false` | 에피소드 기억 활성화. `enableAgentCoreMemory=true` 필요. 에이전트의 과거 경험을 기록 및 검색 |
...
| `authFailureMode` | `fail-open` | Fail-Closed 모드 전환 (`fail-open` / `fail-closed`). `fail-closed`로 권한 취득 실패 시 로그인 차단 |
| `auditLogEnabled` | `false` | 인증 감사 로그. DynamoDB 감사 테이블 (`{prefix}-auth-audit-log`) 생성 |
| `auditLogRetentionDays` | `90` | 감사 로그 보존 일수 (TTL 자동 삭제) |
| `healthCheckEnabled` | `true` | LDAP 헬스 체크 (`ldapConfig` 지정 시). EventBridge 5분 간격 + CloudWatch Alarm |
| `enableAgentRegistry` | `false` | Agent Registry 통합 활성화. Agent Directory에 Registry 탭 추가 |
| `agentRegistryRegion` | 배포 리전 | Agent Registry API 호출 대상 리전. 지원: us-east-1, us-west-2, ap-southeast-2, ap-northeast-1, eu-west-1 |
| `embeddingModel` | `titan-text-v2` | KB 임베딩 모델. `titan-text-v2`(텍스트 전용, 1024차원) 또는 `nova-multimodal`(텍스트/이미지/동영상/오디오, 1024차원). 변경 시 KB 재생성 및 전체 데이터 재수집 필요 |
| `multimodalKbMode` | `replace` | KB 구성 모드. `replace`(단일 KB) 또는 `dual`(텍스트 전용 KB + 멀티모달 KB 병렬 운용). 단계적 마이그레이션에 `dual` 권장 |
...
> **⚠️ 중요**: `cleanup-all.sh`는 모든 CDK 스택을 삭제합니다. Managed AD(`adPassword` 설정 시)가 삭제되면 IAM Identity Center의 ID 소스에 영향을 줄 수 있습니다. 삭제 전에 다음을 확인하세요:
> - Identity Center의 ID 소스가 Managed AD로 설정되어 있지 않은지
> - IAM 사용자 콘솔 접근이 활성화되어 있는지 (백업용)
...
| 증상 | 원인 | 해결 |
|------|------|------|
| Registry 탭이 표시되지 않음 | `enableAgentRegistry=false` 또는 미설정 | `cdk.context.json`에서 `enableAgentRegistry: true`를 설정하고 재배포 |
| Registry 검색 타임아웃 | AgentCore Registry API 연결 실패 | `agentRegistryRegion`이 지원 리전인지 확인. 크로스 리전 시 타임아웃 15초 |
| Registry 리전 미지원 경고 | 배포 리전이 Agent Registry 미지원 | `agentRegistryRegion`에 지원 리전(us-east-1, us-west-2, ap-southeast-2, ap-northeast-1, eu-west-1)을 지정 |

### 멀티모달 RAG 검색

| 증상 | 원인 | 해결 |
|------|------|------|
| 멀티모달 검색 결과 없음 | `embeddingModel`이 `titan-text-v2`(기본값) | `cdk.context.json`에서 `embeddingModel: "nova-multimodal"` 설정 후 재배포. KB 재생성 및 전체 데이터 재수집 필요 |
| Nova Multimodal 리전 미지원 | `nova-multimodal`은 us-east-1, us-west-2만 지원 | 지원 리전에 배포하거나 `titan-text-v2` 사용 |

### Guardrails Organizational Safeguards

| 증상 | 원인 | 해결 |
|------|------|------|
| Guardrails 상태 배지가 표시되지 않음 | `enableGuardrails=false` 또는 Lambda 환경 변수 `GUARDRAILS_ENABLED` 미설정 | `cdk.context.json`에서 `enableGuardrails: true` 설정 후 재배포 |
| "⚠️ 확인 불가" 배지 표시 | Guardrails API 타임아웃(5초 초과) 또는 5xx 오류 | CloudWatch 로그에서 오류 세부 정보 확인. Fail-Open 전략으로 채팅 기능은 계속 동작 |
| GuardrailsAdminPanel이 표시되지 않음 | 관리자 역할이 아니거나 `enableGuardrails=false` | 관리자 역할로 로그인하고 `enableGuardrails=true` 확인 |
| "조직 가드레일: 확인 불가" 표시 | Organizational Safeguards 감지 API 실패 | Lambda 실행 역할에 `bedrock:ListGuardrails`, `bedrock:GetGuardrail` 권한 확인 |
| `guardrailsConfig` 설정이 반영되지 않음 | `enableGuardrails=false` 상태에서 `guardrailsConfig` 설정 | `enableGuardrails=true`를 먼저 설정. `enableGuardrails=false` 시 `guardrailsConfig`는 무시됨 |
| CloudWatch 대시보드에 Guardrails 섹션 없음 | `enableMonitoring=false` 또는 `enableGuardrails=false` | 둘 다 `true`로 설정 후 재배포 |
...
- **비용 최적화**: `enableAgent: true` 시 기본 활성화 (`enableMultiAgent: true`). Bedrock Agent는 대기 비용 $0. 멀티Agent 모드로 실제 채팅 시에만 토큰 소비 3-6배 증가
...
![통합 3모드 토글](docs/screenshots/v4-multi-agent-mode-ja.png)

#### Agent Directory — Teams 탭 + 템플릿 갤러리

Agent Directory에 Teams 탭이 포함되어 있으며, 템플릿 갤러리에서 원클릭으로 Team을 생성할 수 있습니다.

![Teams 탭 + 템플릿 갤러리](docs/screenshots/v4-teams-gallery-ko.png)
