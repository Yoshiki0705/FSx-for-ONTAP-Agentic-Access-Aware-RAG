...
export interface EmbeddingBatchComputeEnvironment {
  ...
  /** インスタンスタイプ */
  readonly instanceTypes: string[]; // ['m5.large', 'm5.xlarge']
  
  /** 最小vCPU */
  readonly minvCpus: number; // 0
  
  /** 最大vCPU */
  readonly maxvCpus: number; // 1000
...
export interface EmbeddingFsxMountConfig {
  ...
  /** SMB/CIFS マウントポイント */
  readonly mountPoint: string; // '/mnt/fsx'
...
export interface EmbeddingEcsCluster {
  ...
  /** 容量プロバイダー */
  readonly capacityProviders: string[]; // ['EC2']
...
export interface EmbeddingEcsLogging {
  /** ログドライバー */
  readonly logDriver: string; // 'awslogs'
...
export interface EmbeddingEcsService {
  ...
  /** 起動タイプ */
  readonly launchType: ecs.LaunchType; // EC2
 ...
 */
export type EmbeddingScalingMetricType = 
  | 'CPUUtilization'
  | 'MemoryUtilization'
  | 'NetworkIn'
  | 'NetworkOut'
