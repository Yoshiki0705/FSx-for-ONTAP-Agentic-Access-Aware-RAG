...
export interface EmbeddingStackProps extends cdk.StackProps {
  readonly config: EmbeddingStackConfig; // 型安全な統合設定オブジェクト
  readonly projectName: string; // プロジェクト名（コスト配布用）
  readonly environment: string; // 環境名（コスト配布用）
  readonly vpc?: any; // VPC（NetworkingStackから）
  readonly privateSubnetIds?: string[]; // プライベートサブネットID
  readonly publicSubnetIds?: string[]; // パブリックサブネットID
  readonly s3BucketNames?: { [key: string]: string }; // S3バケット名（DataStackから）
