...
### 5-3a. マルチエージェントの ROI シナリオ: 四半期レビュー資料の自動生成

**ビジネスシナリオ**: 経営会議向けの四半期レビュー資料を作成する。通常は財務データ収集（1時間）+ エンジニアリング進捗確認（30分）+ 資料作成（2時間）= 合計 3.5 時間かかる作業。

**admin ユーザーでサインイン（マルチ Agent モード）:**

```
質問: 第4四半期の経営レビュー資料を作成してください。
      財務実績（売上・利益・コスト）、エンジニアリングのロードマップ進捗、
      セキュリティ状況を含めてください。
```

**期待結果**:
- Supervisor Agent が Financial Agent + Project Agent に並行で質問を振り分け
- Financial Agent: 予算実績報告書から売上・利益データを取得
- Project Agent: エンジニアリングロードマップから進捗状況を取得
- Supervisor が統合レポートを生成

**ROI 比較**:

| 方式 | 所要時間 | コスト | 品質 |
|------|---------|--------|------|
| 手動作成 | 3.5 時間 | 人件費 | 担当者依存 |
| Single Agent × 3 回質問 | 30 秒 × 3 | ~$0.03 | 個別回答、統合は手動 |
| Multi-Agent 1 回 | 45 秒 | ~$0.10 | 統合レポート自動生成 |

> **ポイント**: マルチエージェントのコスト（トークン 3-6 倍）は、人間の統合作業を省略できることで十分に回収できます。
