...
![画像アップロードゾーン](docs/screenshots/kb-mode-image-upload-zone.png)
...
![AgentCore Memory サイドバー](docs/screenshots/agent-mode-agentcore-memory-sidebar.png)
...
| 3 | SecurityStack | ap-northeast-1 | Cognito User Pool, Client, SAML IdP + Cognito Domain（AD Federation有効時）, AD Sync Lambda（オプション） | 認証・認可 |
...
| L5: ユーザー認証 | Cognito JWT / SAMLフェデレーション | ユーザーレベルの認証・認可 |
...
```
ADユーザー → CloudFront UI → 「ADでサインイン」ボタン
  → Cognito Hosted UI → SAML IdP (AD) → AD認証
  → Cognito ユーザー自動作成
  → Post-Auth Trigger → AD Sync Lambda → DynamoDB SIDデータ登録
  → OAuth Callback → セッションCookie → チャット画面
```
...
2. AWS IAM Identity Centerを有効化し、Managed ADをIDソースとして設定
3. IAM Identity CenterでCognito User Pool向けSAMLアプリケーションを作成（または `samlMetadataUrl` で外部IdPを指定）
4. デプロイ完了後、CloudFront URLを `cloudFrontUrl` に設定して再デプロイ
5. CloudFront UIの「ADでサインイン」ボタンからAD認証を実行
