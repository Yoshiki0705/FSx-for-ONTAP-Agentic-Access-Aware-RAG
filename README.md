...
| AgentCore Web Search | GA（2026年6月） | AwsCustomResource（built-in connector target） | Gateway に Web Search MCP ツールを追加。データは AWS 環境内で処理（zero data egress）。`enableWebSearch=true` で有効化 |
