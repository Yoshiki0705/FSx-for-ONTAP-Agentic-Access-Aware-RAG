...
| 業種 | 顧客の課題 | 本システムでの解決 | 期待効果 |
|------|-----------|------------------|---------|
| **製造** | 設計図面・技術文書が部門間で散在し、必要な情報に辿り着けない | 部門×プロジェクト×機密レベルの権限付きRAG検索 | 設計レビュー準備時間60%削減 |
| **金融** | 規制文書・内部レポートへのアクセス管理が手動で漏えいリスク | 役職×部署の自動権限フィルタリング | コンプライアンス確認工数50%削減 |
| **公共** | 政策文書・内部資料の検索に時間がかかり、部局間連携が遅い | 部局×職位×公開/非公開の権限制御付き検索 | 政策立案の情報収集時間70%削減 |
| **医療** | 手順書・研究資料が診療科ごとに分断、横断検索ができない | 診療科×職種の権限付き横断検索 | 臨床判断支援の情報取得時間50%削減 |
| **法務** | 契約書・判例の案件別管理が煩雑、クライアント情報の分離が困難 | 案件×担当者×クライアント分離のRAG | 契約レビュー時間40%削減 |
| **教育** | 教材・研究資料が学部ごとに閉じ、共同研究での情報共有が困難 | 学部×教職員/学生×研究室の権限制御 | 研究資料検索時間55%削減 |
| **保険** | 査定資料・不正検知レポートへのアクセス管理が複雑 | 部門×案件×機密レベルの自動フィルタリング | 査定業務効率30%向上 |

> 業種別デモデータパック（7業種×各5ドキュメント）を同梱しています。→ [demo-data/industry-packs/](demo-data/industry-packs/)

---

## Responsible AI ステートメント

本システムのAI出力は**補助的なシグナル（assistive signal）**であり、最終的な意思決定を代替するものではありません。

- AI回答は参考情報です。業務上の最終判断は、必ず人間の責任者が行ってください
- 権限フィルタリングは技術的なアクセス制御であり、法的・コンプライアンス上の判断を代替しません
- 規制対象ワークロード（医療、金融、公共）での利用には、顧客固有の法務・コンプライアンス評価が必要です
- 本リポジトリのサンプルデータは全て架空のデモデータです。本番データの投入前に[安全な実験ガイド](docs/safe-experimentation-guide.md)を確認してください
- 監査証跡の保全、データオーナーの特定、承認フローの設計は顧客の責任範囲です

詳細は [ガバナンス・監査設計](docs/governance-and-audit.md) および [脅威モデル](docs/threat-model.md) を参照してください。
...
┌─────────────┐    ┌─────────────┐    ┌─────────────┐    ┌─────────────┐
│  1. 体験     │    │  2. 検証     │    │  3. 評価     │    │  4. 判断     │
│  (1日)       │───▶│  (2-4週間)   │───▶│  (1週間)     │───▶│  (Go/No-Go)  │
└─────────────┘    └─────────────┘    └─────────────┘    └─────────────┘
│                  │                  │                  │
├ デモデータで      ├ 実データ投入      ├ KPI計測          ├ ビジネススポンサー
│ 動作確認         ├ IdP接続          ├ ユーザー評価      │ による判定
├ 権限差異を体験    ├ 権限設計検証      ├ 精度・速度・      ├ 次フェーズ条件
├ 90分ワークショップ├ 業種別データ投入   │ セキュリティ確認   │ の確認
│                  │                  │                  │
▼                  ▼                  ▼                  ▼
[ワークショップ     [安全な実験         [評価フレーム       [PoC成功基準
 ガイド]            ガイド]             ワーク]            テンプレート]
```

| フェーズ | 所要時間 | AWS コスト | 成果物 | ドキュメント |
|---------|---------|-----------|--------|-------------|
| 1. 体験 | 1日 | ~$10 | 動作確認完了 | [PoC ワークショップガイド](docs/poc-workshop-guide.md) |
| 2. 検証 | 2-4週間 | ~$430/月 | 実データでの動作確認 | [安全な実験ガイド](docs/safe-experimentation-guide.md)、[データレディネス評価](docs/data-readiness-assessment.md) |
| 3. 評価 | 1週間 | — | KPI計測結果 | [RAG/Agent 評価フレームワーク](docs/evaluation.md) |
| 4. 判断 | — | — | Go/No-Go判定 | [PoC 成功基準テンプレート](docs/poc-success-criteria-template.md) |
| 5. 本番化 | 2-4週間 | 構成依存 | 本番環境 | [本番化チェックリスト](docs/production-readiness-checklist.md) |
...
| 11 | Smart Routing | クエリ複雑度に基づく3層自動モデル選択。simple（短い事実確認）→ Haiku、complex（長い分析的クエリ）→ Sonnet、full-context（ドキュメント分析意図 + 大量コンテキスト）→ Claude Opus 4.8。GPT-5.5 手動選択オプション（ModelAccessVerifier による可用性検証）。Document Analysis Intent 検出（JP/EN キーワード）。CloudWatch EMF メトリクス（`SmartRouting` 名前空間、`RoutingTier` ディメンション）。サイドバーにON/OFFトグル + 「⚡自動」ボタン | WebAppStack |
...
| 14 | OIDC/LDAP Federation + ONTAP Name-Mapping | OIDC IdP（Auth0/Keycloak/Okta）連携、LDAP直接クエリ（OpenLDAP/FreeIPA）によるUID/GID自動取得、ONTAP REST API name-mapping（UNIX→Windowsユーザー対応付け）。設定駆動の自動有効化。`oidcProviderConfig` + `ldapConfig` + `ontapNameMappingEnabled`で有効化。**Phase 2拡張**: マルチOIDC IdP（`oidcProviders`配列）、OIDCグループベースドキュメントアクセス制御（`allowed_oidc_groups`）、LDAP TLS証明書検証（`tlsCaCertArn`）、トークンリフレッシュ/セッション管理、Fail-Closedモード（`authFailureMode`）、LDAPヘルスチェック（EventBridge + CloudWatch Alarm）、認証監査ログ（`auditLogEnabled`） | SecurityStack |
| 15 | Agent Registry 統合 | Agent Directory に AWS Agent Registry（AgentCore）タブを追加。組織内の Agent・ツール・MCP サーバーをセマンティック検索・インポート・パブリッシュ。承認ワークフロー対応。クロスリージョンアクセス（`agentRegistryRegion`）。`enableAgentRegistry=true` で有効化 | AIStack, WebAppStack |
...
| 19.1 | Web Search | AgentCore Gateway 経由の Web 検索ツール（us-east-1 専用 `DemoWebSearchGatewayStack`）。エージェントがリアルタイムの Web 情報を引用（URL・タイトル・公開日）付きで取得。FSx for ONTAP ドキュメントだけでは回答できない場合の補完情報源。Zero data egress（AWS 環境内完結）。機構 A（Claude Platform）と機構 C（AgentCore Gateway MCP）の 2 パス自動フォールバック。`enableWebSearch=true` で有効化（`enableAgentCoreGateway=true` が前提条件） | WebSearchGatewayStack (us-east-1), WebAppStack |
...
| 機能 | API ステータス | 実装方式 | 注意事項 |
|------|--------------|---------|---------|
| Agent Registry | Preview（2026年4月） | SigV4 署名 HTTP + REST パスマッピング | SDK コマンド（`search_registry_records` 等）は boto3/CLI で利用可能。Node.js は SigV4 HTTP。事前に `create_registry` でレジストリ作成が必要。`agentRegistryArn` で ARN を指定 |
...
| Policy + Guardrails 統合 | GA（2026年6月、東京対応） | CfnPolicyEngine + CfnPolicy（Cedar）| Gateway の policyEngineConfiguration に Guardrails を統合。ツール入出力をリアルタイム評価。`policyEngineMode=LOG_ONLY` でテスト後 `ENFORCE` に切替推奨 |
| AgentCore Web Search | GA（2026年6月、us-east-1） | AwsCustomResource（built-in connector target）+ SigV4 MCP クライアント | us-east-1 に専用 `DemoWebSearchGatewayStack`（`crossRegionReferences` で WebApp に URL 連携）。機構 C: `gateway-client.ts`（SigV4 署名 HTTP POST、5秒タイムアウト、Graceful degradation）。機構 A（Claude Platform）にフォールバック。`enableWebSearch=true` で有効化 |
| InvokeGuardrailChecks API | GA（2026年6月） | AWS SDK v3（BedrockRuntimeClient） | Guardrail リソース不要で個別セーフガードをスコア付き呼び出し。RAG チャンクのインライン安全性チェックに使用 |
| AgentCore Optimization | Preview（2026年5月、GA一部14リージョン） | AwsCustomResource（Configuration Bundle）+ agentcore CLI | CDK は Config Bundle + IAM ロールを構築。Recommendations/A/B テストは CLI/SDK で実行。CloudWatch Transaction Search 有効化が前提 |
...
v4.0.0 の新機能（Guardrails 管理パネル、Agent Registry タブ、AgentCore Policy セクション等）の UI 表示は、Lambda 環境変数からランタイムで取得するフィーチャーフラグ API（`/api/config/features`）で制御されます。CDK パラメータで機能を有効化すると、対応する Lambda 環境変数が設定され、フロントエンドに自動的に反映されます。

**マルチエージェント協調:**

`enableAgent=true` の場合、`enableMultiAgent` はデフォルトで有効化されます（明示的に `false` を設定した場合のみ無効化）。Bedrock Agent は待機コストゼロのため、有効化しても追加ランニングコストは発生しません。マルチ Agent モードで実際にチャットした場合のみトークン消費が 3-6 倍になります。

**npm 依存関係（v4.0.0 で追加）:**

`docker/nextjs/package.json` に以下のパッケージが必要です（SigV4 署名に使用）:
```bash
cd docker/nextjs
...
ヘッダーに統合3モードトグル（KB / シングルAgent / マルチAgent）を配置。サイドバーにはユーザー情報、アクセス権限（ディレクトリ名・読み取り/書き込み権限）、チャット履歴設定、システム管理（リージョン・モデル選択・Smart Routing・権限制御）を表示。チャットエリアには目的別カード14枚がグリッド表示されます。

![KBモード カードグリッド](docs/screenshots/v4-kb-mode-ja.png)

### Agentモード — カードグリッド + サイドバー

シングルAgentモードではヘッダーにAgent選択ドロップダウンが表示され、利用可能なAgent一覧とAgent Directoryリンクにアクセスできます。サイドバーにはAgent情報、アクセス権限（ディレクトリ名・読み取り/書き込み権限）、チャット履歴設定、システム管理セクションがあります。マルチAgentモードではSupervisor Agentのみが選択可能です。

![Agentモード カードグリッド](docs/screenshots/v4-agent-mode-ja.png)
...
![Agent Directory](docs/screenshots/v4-agent-directory-registry-ja.png)
...
![チャット応答 + Citation](docs/screenshots/v4-kb-chat-response-ja.png)
...
| L6: データ認可 | SID / UID+GID / OIDCグループフィルタリング | ドキュメントレベルのアクセス制御。Fail-Closedモード（`authFailureMode=fail-closed`）で権限取得失敗時のサインインブロックも可能 |
...
### ⚠️ 既存AWS環境への影響に関する重要な注意事項

本CDKプロジェクトは以下のAWSリソースを作成・変更します。**既存の本番環境やチーム共有環境にデプロイする場合は、事前に影響を確認してください。**

#### 本システムの認証とAWSマネジメントコンソールの認証は完全に独立しています

本RAGシステムの認証方式（OIDC / LDAP / メールパスワード）と、AWSマネジメントコンソールへのアクセス（IAM Identity Center / IAMユーザー）は**完全に独立した仕組み**です。RAGシステムの認証方式をどのように変更しても、マネジメントコンソールへのアクセスには影響しません。

```
┌──────────────────────────────┐    ┌──────────────────────────────┐
│  AWSマネジメントコンソール       │    │  RAGシステム（チャッ UI）       │
│                              │    │                              │
│  認証： IAM Identity Center   │    │  認証： Cognito User Pool     │
│        または、IAMユーザー      │    │    ├ OIDC (Auth0/Okta等)     │
│                              │    │   ├ SAML (AD Federation)     │
│  ← 本CDKでは、変更しない        │    │    └ メール/パスワード          │
│                              │    │                              │
│  お互いに独立 ─────────────────┼────┤  ← 本CDKで設定・変更する        │
└──────────────────────────────┘    └──────────────────────────────┘
```
...
`adPassword`を設定するとAWS Managed Microsoft ADが作成されますが、**ADを作成しただけではIdentity Centerには影響しません**。

問題が発生するのは、以下の**手動操作を行った場合のみ**です：

1. CDKでManaged ADを作成（`adPassword`設定）
2. **ユーザーが手動で**Identity CenterのIDソースを「Identity Centerディレクトリ」→「Active Directory」に変更
3. Identity CenterがManaged ADのユーザーをIDソースとして使用開始
4. `cdk destroy`でManaged ADが削除される
5. → Identity CenterのIDソースが消失し、コンソールにアクセスできなくなる

**対策:**
- Identity CenterのIDソースは「Identity Centerディレクトリ」（デフォルト）のまま変更しない
- IAMユーザーでのコンソールアクセスをバックアップとして維持する
- `cdk destroy`前にIdentity CenterのIDソース設定を確認する

**影響確認コマンド:**
```bash
# IAM Identity Centerのインスタンスとユーザーを確認
...
| 変更点 | 影響 | 確認方法 |
|--------|------|---------|
| ヘッダーの統合3モードトグル（KB/シングルAgent/マルチAgent） | 既存のKB/Agent 2ボタントグルとSingle/Multiトグルが1つに統合。URLクエリパラメータ（`?mode=agent`, `?mode=multi-agent`）は互換性維持 | ブラウザでモード切替が正常に動作することを確認 |
| ModelIndicatorのヘッダーからの削除 | モデル選択はサイドバーのシステム管理セクションに集約。ヘッダーからのモデル変更は不可 | サイドバーのシステム管理からモデル変更が可能なことを確認 |
| Agent選択ドロップダウンのヘッダー昇格 | Agent DirectoryリンクがヘッダーのユーザーメニューからAgent選択ドロップダウン内に移動 | Agentモードでヘッダーの「Agent選択」からAgent DirectoryにアクセスできることをConfirm |
| サイドバーのアクセス権限セクション追加（Agentモード） | Agentモードのサイドバーにもディレクトリ名・読み取り/書き込み権限が表示 | Agentモードのサイドバーにアクセス権限が表示されることを確認 |
| Agent一覧のモード別フィルタリング | シングルAgentモードではSupervisor以外、マルチAgentモードではSupervisorのみ表示 | 各モードでAgent選択ドロップダウンの内容が異なることを確認 |
| CDK AIスタック: SupervisorAgent `agentCollaboration` | `DISABLED` → `SUPERVISOR_ROUTER` に変更。既にコラボレーターが紐付いている環境では必須 | `cdk diff perm-rag-demo-demo-AI` でSupervisorAgentの変更を確認 |
| localStorage永続化キー `header-store` | サイドバー開閉状態とチャットモードがlocalStorageに保存。ブラウザキャッシュクリアで初期化 | 初回アクセス時にKBモードがデフォルトで表示されることを確認 |
...
# 2. AIスタックのSupervisorAgent変更を確認（DISABLED → SUPERVISOR_ROUTER）
# ⚠️ AIスタックのロールバック失敗が発生した場合:
aws cloudformation continue-update-rollback \
  --stack-name perm-rag-demo-demo-AI \
  --region ap-northeast-1 \
  --resources-to-skip SupervisorAgent

# 3. WebAppスタックのデプロイ（Dockerイメージ更新）
./development/scripts/deploy-webapp.sh

# 4. ブラウザでの動作確認
# - KBモード → シングルAgent → マルチAgent の切替がエラーなく動作
# - Agent選択ドロップダウンにAgent一覧が表示
# - サイドバーにアクセス権限が表示
```
...
> **⚠️ 重要**: `adPassword`を設定するとAWS Managed Microsoft ADが作成されます。AD作成自体はIdentity Centerに影響しませんが、Identity CenterのIDソースを手動でManaged ADに変更した場合、`cdk destroy`でADを削除するとIdentity Centerのユーザーが失われます。Identity CenterのIDソースは「Identity Centerディレクトリ」（デフォルト）のまま変更しないでください。
...
| OIDC サインイン `invalid_request` | issuerUrl不一致 | `oidcProviderConfig.issuerUrl` がIdPの `/.well-known/openid-configuration` の `issuer` フィールドと完全一致していることを確認。Auth0は末尾スラッシュ付き（`https://xxx.auth0.com/`）が必要 |
| OIDC サインイン `Attribute cannot be updated` | Cognito User Poolのemail属性が`mutable: false` | CDKの`standardAttributes.email.mutable`が`true`であることを確認。`mutable`はUser Pool作成後に変更不可のため、User Poolの再作成が必要 |
| OIDC IdP手動削除後にCDKデプロイ失敗 | CDKスタック状態不整合 | Cognito IdPを手動で削除・再作成するとCDKスタックの状態と実態が不一致になる。CDKデプロイのみで管理し、手動操作は避ける |
| LDAPヘルスチェックLambdaがタイムアウト | VPC内LambdaがSecrets Managerにアクセスできない | NATゲートウェイまたは`enableVpcEndpoints=true`（Secrets Manager VPCエンドポイント）が必要。`aws lambda invoke --function-name perm-rag-demo-demo-ldap-health-check /tmp/result.json`で手動テスト |
| LDAPヘルスチェックAlarmがALARM状態 | LDAP接続失敗 | CloudWatch Logs（`/aws/lambda/perm-rag-demo-demo-ldap-health-check`）で構造化ログを確認。接続エラー→SG/VPC設定、バインドエラー→パスワード/DN、検索エラー→baseDNを確認 |
| `--exclusively`なしでNetworkingスタック更新失敗 | VPC CrossStack Export依存 | `npx cdk deploy perm-rag-demo-demo-Security perm-rag-demo-demo-WebApp --exclusively`でSecurity/WebAppのみ個別デプロイ |
| メール/パスワードサインイン時に`AD_EC2_INSTANCE_ID is required`エラー | AD未設定時にSID同期が実行される | v3.5.0で修正済み。`AD_TYPE`デフォルト値を`none`に変更し、AD未設定時はSID同期をスキップ。古いLambdaの場合は再デプロイが必要 |
| Fail-Closedモードが発動しない（LDAPユーザー未検出時） | 設計通りの動作 | LDAPユーザー未検出はエラーではなくOIDCクレームへのフォールバック。Fail-Closedが発動するのはLDAP接続タイムアウト、Secrets Manager取得失敗等の致命的エラー時のみ |
| `oidcProviderConfig`→`oidcProviders`移行でCDKデプロイ失敗 | Cognito IdPリソースID競合 | v3.5.0で修正済み。最初のIdPのCDKリソースIDを`OidcIdP`に固定し、移行互換性を維持。古いバージョンの場合はSecurityスタックの`cdk destroy`→再デプロイが必要 |
| ONTAP REST API `User is not authorized` | fsxadminパスワード未設定 | `aws fsx update-file-system --file-system-id <FS_ID> --ontap-configuration '{"FsxAdminPassword":"<PASSWORD>"}'`で設定。Secrets Managerに保存して`ontapAdminSecretArn`に指定 |
...
お客様の既存環境に合わせて、以下の認証方式から選択できます。**どの方式を選んでも、AWSマネジメントコンソールへのアクセスには影響しません。**

| お客様の環境 | 推奨する認証方式 | サインイン画面のボタン | 設定パラメータ |
|-------------|----------------|---------------------|-------------|
| 特になし（まず試したい） | メール/パスワード | フォームのみ | 設定不要（デフォルト） |
| Okta を使っている | OIDC Federation | 「Oktaでサインイン」 | `oidcProviderConfig.providerName: "Okta"` |
| Keycloak を使っている | OIDC + LDAP | 「Keycloakでサインイン」 | `oidcProviderConfig` + `ldapConfig` |
| Entra ID（Azure AD）を使っている | OIDC Federation | 「EntraIDでサインイン」 | `oidcProviderConfig.providerName: "EntraID"` |
| Auth0 を使っている | OIDC Federation | 「Auth0でサインイン」 | `oidcProviderConfig.providerName: "Auth0"` |
| Windows AD を使っている | SAML AD Federation | 「ADでサインイン」 | `enableAdFederation: true` |
| 複数のIdPを同時に使いたい | マルチOIDC | 各IdPのボタンが並列表示 | `oidcProviders` 配列 |

> **サインイン画面のボタン名はカスタマイズ可能です**: `providerName`に設定した名前がそのままボタンに表示されます。例えば`"社内SSO"`と設定すれば「社内SSOでサインイン」と表示されます。
...
> **⚠️ issuerUrlの注意**: `oidcProviderConfig.issuerUrl` はIdPの `/.well-known/openid-configuration` の `issuer` フィールドの値と完全一致させてください。Auth0は末尾スラッシュ付き（`https://xxx.auth0.com/`）、Keycloakは末尾スラッシュなし（`https://keycloak.example.com/realms/main`）です。不一致の場合、Cognitoのトークン検証で `invalid_request` エラーが発生します。

> **⚠️ OIDCフェデレーションの2段階デプロイ**: OIDC構成では`cloudFrontUrl`がOAuthコールバックに必要ですが、初回デプロイ時はCloudFront URLが未知です。以下の手順で2段階デプロイを行ってください:
> 1. `cloudFrontUrl`なしで`cdk deploy`を実行
> 2. WebAppスタック出力からCloudFront URLを取得: `aws cloudformation describe-stacks --stack-name perm-rag-demo-demo-WebApp --query 'Stacks[0].Outputs[?OutputKey==\`CloudFrontUrl\`].OutputValue' --output text`
> 3. `cdk.context.json`に`cloudFrontUrl`を追加して再デプロイ
> 4. OIDC IdP側のAllowed Callback URLsに`https://{cognito-domain}.auth.{region}.amazoncognito.com/oauth2/idpresponse`を設定

##### Phase 2 拡張機能

Phase 2では以下の7つの拡張機能が追加されました。全て `cdk.context.json` のパラメータで制御します。

**マルチOIDC IdP構成（`oidcProviders`配列）:**

`oidcProviderConfig`（単一IdP）に加え、`oidcProviders` 配列で複数のOIDC IdPを同時登録できます。サインイン画面に各IdPのボタンが動的に表示されます。`oidcProviderConfig` と `oidcProviders` は排他的設定です。
...
**Fail-Closedモード（`authFailureMode`）:**

デフォルトはFail-Open（権限取得失敗時もサインイン継続）。高セキュリティ環境では `authFailureMode: "fail-closed"` を設定すると、権限取得失敗時にサインイン自体をブロックします。

> **⚠️ Fail-Closedの発動条件**: LDAPユーザーが見つからない場合はエラーではなく、OIDCクレームのみへのフォールバック動作です（サインインは継続）。Fail-Closedが発動するのは、LDAP接続タイムアウト、Secrets Managerからのパスワード取得失敗、DynamoDB書き込み失敗等の致命的エラー時のみです。

**LDAPヘルスチェック（`healthCheckEnabled`）:**

`ldapConfig` 指定時に自動有効化（デフォルト: `true`）。EventBridge Ruleで5分間隔の定期実行を行い、LDAP接続・バインド・検索の正常性を検証します。失敗時はCloudWatch Alarmで通知します。

> **⚠️ VPCネットワーク要件**: ヘルスチェックLambdaはVPC内に配置されるため、Secrets Manager（バインドパスワード取得）へのアクセスにはNATゲートウェイまたはSecrets Manager VPCエンドポイントが必要です。`enableVpcEndpoints=true`の使用を推奨します。

> **検証方法**: LDAPヘルスチェックが正常に動作しているか確認するには、以下のコマンドを使用します:
> ```bash
> # ヘルスチェックLambdaの手動実行
...
>
> # CloudWatch Logsでヘルスチェック結果を確認
> aws logs tail /aws/lambda/perm-rag-demo-demo-ldap-health-check --region ap-northeast-1 --since 1h
> ```

**監査ログ（`auditLogEnabled`）:**

`auditLogEnabled: true` で認証イベント（サインイン成功/失敗）をDynamoDB監査テーブルに記録します。TTLによる自動削除（デフォルト: 90日）。監査テーブルへの書き込み失敗時もサインインはブロックしません。

**TLS証明書検証（`tlsCaCertArn`, `tlsRejectUnauthorized`）:**

`ldapConfig` 内に `tlsCaCertArn`（Secrets ManagerのCA証明書ARN）と `tlsRejectUnauthorized`（デフォルト: `true`）を指定して、LDAPS接続のカスタムCA証明書検証を制御します。開発環境では `tlsRejectUnauthorized: false` で自己署名証明書を許可できます。

**OIDCグループベースドキュメントアクセス制御:**

ドキュメントメタデータに `allowed_oidc_groups` を設定すると、ユーザーの `oidcGroups` との交差チェックでアクセス制御を行います。SID/UID-GIDマッチ失敗時のフォールバックとしても機能します。

**トークンリフレッシュとセッション管理:**

OIDC認証後のアクセストークンを自動リフレッシュ。有効期限の5分前にバックグラウンドでリフレッシュを実行し、リフレッシュトークン期限切れ時はサインイン画面にリダイレクトします。

**Phase 2 CDKパラメータ:**

| パラメータ | 型 | デフォルト | 説明 |
|-----------|-----|----------|------|
| `oidcProviders` | array | (なし) | 複数OIDC IdP設定配列（`oidcProviderConfig`と排他） |
| `authFailureMode` | string | `fail-open` | 権限取得失敗時の動作（`fail-open` / `fail-closed`） |
| `auditLogEnabled` | boolean | `false` | 認証監査ログDynamoDBテーブルの作成 |
| `auditLogRetentionDays` | number | `90` | 監査ログの保持日数（TTL自動削除） |
| `healthCheckEnabled` | boolean | `true` | LDAPヘルスチェックLambda + EventBridge + CloudWatch Alarm |
| `tlsCaCertArn` | string | (なし) | `ldapConfig`内: LDAPS用カスタムCA証明書のSecrets Manager ARN |
| `tlsRejectUnauthorized` | boolean | `true` | `ldapConfig`内: TLS証明書検証（`false`で自己署名証明書を許可） |
...
| `guardrailsConfig` | *(なし)* | Guardrails 詳細設定（`contentFilters`, `topicPolicies`, `piiConfig`, `contextualGrounding`）。`enableGuardrails=true` 時のみ有効。未設定時はデフォルト構成（全カテゴリ HIGH） |
...
| `enableEpisodicMemory` | `false` | エピソード記憶を有効化。`enableAgentCoreMemory=true` が前提条件。エージェントの過去の経験を記録・検索 |
| `enableVoiceChat` | `false` | 音声チャット（Amazon Nova Sonic）を有効化。Lambda に `bedrock:InvokeModelWithBidirectionalStream` 権限を付与。推定月額: $70〜$100 |
| `enableAgentPolicy` | `false` | AgentCore Policy（エージェント行動制御）を有効化。自然言語ポリシーでエージェントのツール・API アクセスを制限 |
| `policyFailureMode` | `fail-open` | ポリシー評価失敗時の挙動。`fail-open`: エージェント実行を継続、`fail-closed`: エージェント実行をブロック |
...
| `authFailureMode` | `fail-open` | Fail-Closedモード切替（`fail-open` / `fail-closed`）。`fail-closed`で権限取得失敗時にサインインをブロック |
| `auditLogEnabled` | `false` | 認証監査ログ。DynamoDB監査テーブル（`{prefix}-auth-audit-log`）を作成 |
| `auditLogRetentionDays` | `90` | 監査ログの保持日数（TTL自動削除） |
| `healthCheckEnabled` | `true` | LDAPヘルスチェック（`ldapConfig`指定時）。EventBridge 5分間隔 + CloudWatch Alarm |
| `enableAgentRegistry` | `false` | Agent Registry 統合の有効化。Agent Directory に Registry タブを追加 |
| `agentRegistryRegion` | デプロイリージョン | Agent Registry API 呼び出し先リージョン。対応: us-east-1, us-west-2, ap-southeast-2, ap-northeast-1, eu-west-1 |
| `embeddingModel` | `titan-text-v2` | KB 埋め込みモデル。`titan-text-v2`（テキスト専用、1024次元）または `nova-multimodal`（テキスト・画像・動画・音声、1024次元）。変更には KB 再作成と全データ再インジェストが必要 |
| `multimodalKbMode` | `replace` | KB 構成モード。`replace`（単一 KB）または `dual`（テキスト専用 KB + マルチモーダル KB 並行運用）。段階的移行に `dual` を推奨 |
...
> **⚠️ 重要**: `cleanup-all.sh`は全CDKスタックを削除します。Managed AD（`adPassword`設定時）も削除されます。Identity CenterのIDソースを手動でManaged ADに変更していた場合、コンソールにアクセスできなくなります。削除前に以下を確認してください:
> - Identity CenterのIDソースが「Identity Centerディレクトリ」（デフォルト）であること
> - IAMユーザーでのコンソールアクセスが有効であること（バックアップ）
...
### Episodic Memory トラブルシューティング

| 症状 | 原因 | 対処 |
|------|------|------|
| エピソードタブが表示されない | `enableEpisodicMemory=false` または未設定 | `cdk.context.json` で `enableEpisodicMemory: true`（`enableAgentCoreMemory: true` が前提）を設定して再デプロイ |
| エピソード一覧が空 | Background Reflection 未完了 | 会話完了後にエピソードが自動抽出されるまで数分待機。リフレッシュボタンで更新 |
| 類似エピソード参照バッジが表示されない | 類似エピソードが見つからない | エピソードが蓄積されるまで通常通りエージェントを使用 |
| エピソード削除後にリストに復元される | 削除 API エラー | ネットワーク接続を確認。IAM ポリシーに `DeleteMemoryRecord` が含まれていることを確認 |

### Agent Registry 統合

| 症状 | 原因 | 対処 |
|------|------|------|
| Registry タブが表示されない | `enableAgentRegistry=false` または未設定 | `cdk.context.json` で `enableAgentRegistry: true` を設定して再デプロイ |
| Registry 検索がタイムアウト | AgentCore Registry API 接続失敗 | `agentRegistryRegion` が対応リージョンであることを確認。クロスリージョン時はタイムアウト 15 秒 |
| Registry リージョン非対応警告 | デプロイリージョンが Agent Registry 非対応 | `agentRegistryRegion` に対応リージョン（us-east-1, us-west-2, ap-southeast-2, ap-northeast-1, eu-west-1）を指定 |

### マルチモーダル RAG 検索

| 症状 | 原因 | 対処 |
|------|------|------|
| マルチモーダル検索結果が返らない | `embeddingModel` が `titan-text-v2`（デフォルト） | `cdk.context.json` で `embeddingModel: "nova-multimodal"` を設定して再デプロイ。KB 再作成と全データ再インジェストが必要 |
| Nova Multimodal がリージョン非対応 | `nova-multimodal` は us-east-1, us-west-2 のみ対応 | 対応リージョンにデプロイするか、`titan-text-v2` を使用 |
| マルチモーダル UI が表示されない | `MULTIMODAL_ENABLED=false` | `embeddingModel: "nova-multimodal"` で再デプロイ。Lambda 環境変数 `MULTIMODAL_ENABLED=true` が自動設定される |
| Dual KB モードで片方の KB が失敗 | KB インジェストエラーまたはリソース制限 | CloudWatch ログで各 KB のインジェスト状況を確認。正常な KB で検索は継続される |

### Guardrails Organizational Safeguards

| 症状 | 原因 | 対処 |
|------|------|------|
| Guardrails ステータスバッジが表示されない | `enableGuardrails=false` または Lambda 環境変数 `GUARDRAILS_ENABLED` 未設定 | `cdk.context.json` で `enableGuardrails: true` を設定して再デプロイ |
| 「⚠️ チェック不可」バッジが表示される | Guardrails API タイムアウト（5秒超）または 5xx エラー | CloudWatch ログでエラー詳細を確認。Fail-Open 戦略によりチャット機能は継続動作 |
| GuardrailsAdminPanel が表示されない | 管理者ロールでない、または `enableGuardrails=false` | 管理者ロールでサインインし、`enableGuardrails=true` を確認 |
| 「組織ガードレール: 確認不可」と表示される | Organizational Safeguards 検出 API 失敗 | Lambda 実行ロールに `bedrock:ListGuardrails`, `bedrock:GetGuardrail` 権限があることを確認 |
| `guardrailsConfig` 設定が反映されない | `enableGuardrails=false` の状態で `guardrailsConfig` を設定 | `enableGuardrails=true` を先に設定。`enableGuardrails=false` 時は `guardrailsConfig` は無視される |
| CloudWatch ダッシュボードに Guardrails セクションがない | `enableMonitoring=false` または `enableGuardrails=false` | 両方を `true` に設定して再デプロイ |
...
#### 統合3モードトグル — KB / シングルAgent / マルチAgent

ヘッダーに統合3モードトグルを配置。KB（青）、シングルAgent（紫）、マルチAgent（紫）の3モードをワンクリックで切替可能。Agent選択ドロップダウンはAgentモード時のみ表示され、シングルモードでは個別Agent、マルチモードではSupervisor Agentのみが選択可能です。

![統合3モードトグル](docs/screenshots/v4-multi-agent-mode-ja.png)

#### Agent Directory — Teamsタブ + テンプレートギャラリー

Agent DirectoryにTeamsタブが追加され、テンプレートギャラリーからワンクリックでTeamを作成できます。Agent Directoryへはヘッダーの「Agent選択」ドロップダウン内のリンクからアクセスできます。

![Teamsタブ + テンプレートギャラリー](docs/screenshots/v4-teams-gallery-ja.png)
...
#### マルチAgentモード — Agent選択ドロップダウン

マルチAgentモードではSupervisor Agentのみが選択可能。Agent Directoryリンクもドロップダウン内に配置されています。

![マルチAgentモード有効化](docs/screenshots/multi-agent-mode-toggle-enabled-production-ja.png)
...
```jsonc
{
  "enableMultiAgent": true,               // マルチエージェント有効化（enableAgent=true 時はデフォルト: true。待機コストゼロ）
