...
Ce dépôt est un exemple qui déploie un RAG Agentique avec contrôle d'accès alimenté par Amazon Bedrock à l'aide d'AWS CDK, exploitant les données d'entreprise et les permissions d'accès sur Amazon FSx for NetApp ONTAP. En utilisant FSx for ONTAP comme source de données, il implémente la recherche et la génération de réponses en tenant compte des informations ACL / permissions. Pour le magasin de vecteurs, vous pouvez choisir entre Amazon S3 Vectors (par défaut, faible coût) ou Amazon OpenSearch Serverless (haute performance). Il dispose d'une interface utilisateur orientée tâches basée sur des cartes, construite avec Next.js 15 sur AWS Lambda (Lambda Web Adapter), vous permettant de valider une configuration RAG / assistant IA sécurisée pour une utilisation en entreprise.
...
## Vue d'ensemble de l'implémentation (13 perspectives)

L'implémentation de ce système est organisée en 13 perspectives. Pour les détails de chaque élément, consultez [docs/implementation-overview.md](docs/implementation-overview.md).
...
| 3 | SecurityStack | ap-northeast-1 | Cognito User Pool, Client, SAML IdP + Cognito Domain (quand AD Federation activé), AD Sync Lambda (optionnel) | Authentification et autorisation |
...
| L5 : Authentification utilisateur | Cognito JWT / SAML Federation | Authentification et autorisation au niveau utilisateur |
| L6 : Autorisation des données | SID Filtering | Contrôle d'accès au niveau document |
...
2. Activer AWS IAM Identity Center et configurer l'AD géré comme source d'identité
3. Créer une application SAML pour Cognito User Pool dans IAM Identity Center (ou spécifier un IdP externe via `samlMetadataUrl`)
4. Après le déploiement, définir l'URL CloudFront dans `cloudFrontUrl` et redéployer
5. Exécuter l'authentification AD depuis le bouton « Sign in with AD » sur l'interface CloudFront
...
| `existingVolumeId` | ID Volume existant (ex. `fsvol-0123456789abcdef0`) |
...
| Document | Contenu |
|----------|---------|
| [docs/implementation-overview.md](docs/implementation-overview.md) | Description détaillée de l'implémentation (13 perspectives) |
