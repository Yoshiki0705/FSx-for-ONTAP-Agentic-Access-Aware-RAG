...
| 15 | Agent Registry 集成 | 在 Agent Directory 中添加 AWS Agent Registry（AgentCore）标签页。语义搜索、导入和发布组织内的 Agent、工具和 MCP 服务器。支持审批工作流。跨区域访问（`agentRegistryRegion`）。通过 `enableAgentRegistry=true` 启用 | AIStack, WebAppStack |
| 16 | 多模态 RAG 搜索 | 使用 Amazon Nova Multimodal Embeddings 实现文本、图片、视频、音频的跨模态搜索。通过 Embedding Model Registry + KB Config Strategy 模式确保可扩展性。支持 Dual KB 架构（纯文本 + 多模态并行运行）。通过 `embeddingModel: "nova-multimodal"` 选择性启用 | AIStack |
...
| 19 | AgentCore Policy | 通过 AgentCore Policy 实现代理行为控制。使用自然语言策略定义限制代理的工具、API、MCP 服务器访问。3种策略模板（安全优先、成本优先、灵活性优先）。PolicyEvaluationMiddleware（3秒超时、fail-open/fail-closed）。违规日志（EMF 格式）及 CloudWatch 仪表板集成。8种语言 i18n 支持。通过 `enableAgentPolicy=true` 启用 | WebAppStack |
...
| 功能 | API 状态 | 实现方式 | 说明 |
|------|---------|---------|------|
| Agent Registry | Preview（2026年4月） | SigV4 签名 HTTP + REST 路径映射 | SDK 命令（`search_registry_records` 等）可在 boto3/CLI 中使用。Node.js 使用 SigV4 HTTP。需先通过 `create_registry` 创建注册表。通过 `agentRegistryArn` 指定 ARN |
| 多模态 RAG | GA（2025年11月） | AWS SDK v3（BedrockAgentRuntimeClient） | 完全实现。Nova Multimodal Embeddings 仅支持 us-east-1、us-west-2 |
| Guardrails Org Safeguards | GA（2026年4月） | AWS SDK v3（BedrockClient） | 完全实现。Organizational Safeguards 在 AWS Organizations 管理账户中配置 |
| AgentCore Episodic Memory | GA（2025年12月） | AWS SDK v3（BedrockAgentCoreClient） | `episodicMemoryStrategy` 需要 `reflectionConfiguration.namespaces` 参数。未指定时 CreateMemory 返回 "Invalid memory strategy input" 错误 |
| Nova Sonic 语音聊天 | GA（2025年12月） | REST + Bedrock Converse API | Phase 1 实现（基于 REST）。实时双向流式传输需要 API Gateway WebSocket（Phase 2） |
| AgentCore Policy | GA（2026年3月） | SigV4 签名 HTTP（Policy Engine + Gateway 模型） | GA 版本变更为 Policy Engine + Gateway 架构。策略使用 Cedar 语言编写。IAM 操作更新为 `bedrock-agentcore:CreatePolicyEngine` 等 |

**语音聊天 Phase 1 / Phase 2 架构：**

| 项目 | Phase 1（当前） | Phase 2（未来） |
|------|---------------|---------------|
| 通信方式 | REST（POST /api/voice/stream） | WebSocket（API Gateway） |
| 音频处理 | Bedrock Converse API 将语音转文本后输入 RAG 管道 | Nova Sonic InvokeModelWithBidirectionalStream 双向流式传输 |
| 延迟 | 中等（缓冲 + 批处理） | 低（实时流式传输） |
| 权限过滤 | ✅ 应用现有 SID/UID/GID 过滤 | ✅ 应用相同逻辑 |

**v4.0.0 功能标志：**

v4.0.0 新功能（Guardrails 管理面板、Agent Registry 选项卡、AgentCore Policy 部分等）的 UI 显示由功能标志 API（`/api/config/features`）控制，该 API 在运行时从 Lambda 环境变量中获取标志。当您通过 CDK 参数启用某项功能时，相应的 Lambda 环境变量会被设置并自动反映到前端。

**多智能体协作：**

当 `enableAgent=true` 时，`enableMultiAgent` 默认启用（仅在显式设置为 `false` 时才禁用）。Bedrock Agents 没有待机成本，因此启用多智能体模式不会产生额外的运行费用。令牌消耗仅在实际使用多智能体模式聊天时才会增加 3-6 倍。

**npm 依赖（v4.0.0 新增）：**

`docker/nextjs/package.json` 需要以下包（用于 SigV4 签名）：
```bash
cd docker/nextjs
...
![Agent Directory](docs/screenshots/v4-agent-directory-registry-zh-CN.png)
...
| `guardrailsConfig` | *（无）* | Guardrails 详细配置（`contentFilters`, `topicPolicies`, `piiConfig`, `contextualGrounding`）。仅在 `enableGuardrails=true` 时有效。未设置时使用默认配置（所有类别 HIGH） |
...
| `enableEpisodicMemory` | `false` | 启用情景记忆。需要 `enableAgentCoreMemory=true`。记录和搜索代理的过去经验 |
...
| `enableAgentRegistry` | `false` | 启用 Agent Registry 集成。在 Agent Directory 中添加 Registry 标签页 |
| `agentRegistryRegion` | 部署区域 | Agent Registry API 调用目标区域。支持: us-east-1, us-west-2, ap-southeast-2, ap-northeast-1, eu-west-1 |
...
| 症状 | 原因 | 解决方案 |
|------|------|---------|
| Registry 标签页不显示 | `enableAgentRegistry=false` 或未设置 | 在 `cdk.context.json` 中设置 `enableAgentRegistry: true` 并重新部署 |
| Registry 搜索超时 | AgentCore Registry API 连接失败 | 确认 `agentRegistryRegion` 为支持的区域。跨区域超时为 15 秒 |
| Registry 区域不支持警告 | 部署区域不支持 Agent Registry | 将 `agentRegistryRegion` 设置为支持的区域（us-east-1, us-west-2, ap-southeast-2, ap-northeast-1, eu-west-1） |

### Guardrails Organizational Safeguards

| 症状 | 原因 | 解决方案 |
|------|------|---------|
| Guardrails 状态徽章未显示 | `enableGuardrails=false` 或 Lambda 环境变量 `GUARDRAILS_ENABLED` 未设置 | 在 `cdk.context.json` 中设置 `enableGuardrails: true` 并重新部署 |
| 显示"⚠️ 无法检查"徽章 | Guardrails API 超时（>5秒）或 5xx 错误 | 检查 CloudWatch 日志获取错误详情。通过 Fail-Open 策略聊天功能继续运行 |
| GuardrailsAdminPanel 未显示 | 非管理员角色或 `enableGuardrails=false` | 使用管理员角色登录并确认 `enableGuardrails=true` |
| 显示"组织护栏：无法确认" | Organizational Safeguards 检测 API 失败 | 确认 Lambda 执行角色具有 `bedrock:ListGuardrails`, `bedrock:GetGuardrail` 权限 |
| `guardrailsConfig` 设置未生效 | 在 `enableGuardrails=false` 状态下设置了 `guardrailsConfig` | 先设置 `enableGuardrails=true`。`enableGuardrails=false` 时 `guardrailsConfig` 被忽略 |
| CloudWatch 仪表板中无 Guardrails 部分 | `enableMonitoring=false` 或 `enableGuardrails=false` | 将两者都设置为 `true` 并重新部署 |
...
- **成本优化**：`enableAgent: true` 时默认启用（`enableMultiAgent: true`）。Bedrock Agent 待机成本 $0。仅在多Agent模式实际聊天时令牌消耗增加 3-6 倍
...
![统一 3 模式切换](docs/screenshots/v4-multi-agent-mode-ja.png)

#### Agent Directory — Teams 标签页 + 模板库

Agent Directory 包含 Teams 标签页，通过模板库可一键创建 Team。

![Teams 标签页 + 模板库](docs/screenshots/v4-teams-gallery-zh-CN.png)
