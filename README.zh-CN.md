# 基于 Amazon FSx for NetApp ONTAP 的权限感知 RAG 系统
...
| 3 | SecurityStack | ap-northeast-1 | Cognito User Pool, Client, SAML IdP + Cognito Domain (when AD Federation enabled), AD Sync Lambda (optional) | 认证与授权 |
...
| 层级 | 技术 | 用途 |
|------|------|------|
| L1: 网络 | CloudFront Geo Restriction | 地理访问限制（默认：仅日本） |
...
| L5: 用户认证 | Cognito JWT / SAML Federation | 用户级认证与授权 |
| L6: 数据授权 | SID Filtering | 文档级访问控制 |
...
2. 启用 AWS IAM Identity Center 并将托管 AD 配置为身份源
3. 在 IAM Identity Center 中为 Cognito User Pool 创建 SAML 应用程序（或通过 `samlMetadataUrl` 指定外部 IdP）
4. 部署后，在 `cloudFrontUrl` 中设置 CloudFront URL 并重新部署
5. 从 CloudFront UI 上的"Sign in with AD"按钮执行 AD 认证
...
> **Ingestion Job 配额和设计考虑**：约束包括每个作业 100GB/每个文件 50MB、不支持对同一 KB 的并行同步、StartIngestionJob API 速率 0.1 req/sec（每 10 秒一次）等。包括定期同步调度方法在内的详情，请参阅 [docs/stack-architecture-comparison.md](docs/stack-architecture-comparison.md#bedrock-kb-ingestion-job--クォータと設計考慮点)。
