...
| 3 | SecurityStack | ap-northeast-1 | Cognito User Pool, Client, SAML IdP + Cognito Domain（啟用 AD Federation 時）, AD Sync Lambda（可選） | 認證與授權 |
...
| L5: 使用者認證 | Cognito JWT / SAML Federation | 使用者層級的認證與授權 |
| L6: 資料授權 | SID Filtering | 文件層級的存取控制 |
...
2. 啟用 AWS IAM Identity Center 並將受管 AD 配置為身分來源
3. 在 IAM Identity Center 中為 Cognito User Pool 建立 SAML 應用程式（或透過 `samlMetadataUrl` 指定外部 IdP）
4. 部署後，在 `cloudFrontUrl` 中設定 CloudFront URL 並重新部署
5. 從 CloudFront UI 上的「Sign in with AD」按鈕執行 AD 認證
